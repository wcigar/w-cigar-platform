---
name: "source-command-wcigar-git-status"
description: "雙 repo git 狀態紅綠燈 — 揪「以為 vs 實際」差距，預防多 push 點 / stash 失敗 / lock 殘留"
---

# source-command-wcigar-git-status

Use this skill when the user asks to run the migrated source command `wcigar-git-status`.
...
**讀**（先做，不要寫）：
```powershell
$f = "$env:USERPROFILE\.Codex\wcigar-git-status-last-run.timestamp"
...
Set-Content -Path "$env:USERPROFILE\.Codex\wcigar-git-status-last-run.timestamp" -Value $ts -Encoding utf8
```
...
## 📦 dealer (~/w-cigar-dealer)
🟢 branch: main
🟢 HEAD: d16c727 (2026-05-02 17:25) feat: 安裝 W Cigar Codex slash commands 完整 7 件套
...
- ✅ 例外：寫入 `~/.Codex/wcigar-git-status-last-run.timestamp`（指令自己的紀錄檔）
