              ...
              {!collapsed.has(cat) && catItems.map(item => (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '1px dashed var(--border)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                    ...
                    <input type="text" inputMode="numeric" pattern="[0-9]*" min="0" value={safeEdits[item.id] ?? item.safe_stock ?? ''} onChange={e => setSafeEdits(p => ({ ...p, [item.id]: e.target.value }))} style={{ width: 60, fontSize: 14, fontFamily: 'var(--font-mono)', fontWeight: 700, textAlign: 'center', padding: '6px 4px', borderColor: safeEdits[item.id] != null && Number(safeEdits[item.id]) !== (item.safe_stock || 0) ? 'var(--gold)' : undefined }} />
        ...
        <div>
          <div className="card" style={{ padding: 12, marginBottom: 12, borderColor: 'var(--border-gold)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginBottom: 8 }}>
              ...
              <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="安全值" value={batchSafe} onChange={e => setBatchSafe(e.target.value)} style={{ fontSize: 11, padding: '6px 4px', fontFamily: 'var(--font-mono)' }} />
            </div>
                    ...
                    return (
                      <div key={item.id} className="card" style={{ padding: 12, marginBottom: 4, borderColor: pct === 0 ? 'rgba(196,77,77,.4)' : 'rgba(245,158,11,.3)' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <input type="text" inputMode="numeric" pattern="[0-9]*" min="0" value={purchaseEdits[item.id] ?? item.need} onChange={e => setPurchaseEdits(p => ({ ...p, [item.id]: e.target.value }))} style={{ width: 60, fontSize: 16, fontFamily: 'var(--font-mono)', fontWeight: 700, textAlign: 'center', padding: '4px', color: 'var(--gold)' }} />
