...
export default function Payroll() {
  ...
  async function load() {
    ...
    const s = month + '-01'
    // ✅ 當月：截止到今天；過去月份：截止到月底
    const e = isCurrentMonth ? today : format(endOfMonth(new Date(month + '-01')), 'yyyy-MM-dd')
  ...
  function getAttendance(eid) {
    ...
    let otTotalMin = 0, otDetails = []
    let lateDetails = [], earlyDetails = []
    es.forEach(s => {
      const v = s.shift || ''
      if (v === '早班' || v === '晚班') {
        work++
    ...
    return { work, sick, personal, off, special, total: es.length, lateCount, lateMinutes, lateDetails, earlyCount, earlyMinutes, earlyDetails, otTotalMin, otDetails }
  }
  ...
  function printPayslip() {
    ...
    const cutoffLabel = isCurrentMonth ? `（截至 ${todayDay} 日）` : ''
...
<h1>W Cigar Bar — ${payslip.month} 薪資條${cutoffLabel}</h1>
...
${p.att.earlyCount ? `<div class="r rd"><span>早退</span><span>${p.att.earlyCount} 次 · 共${p.att.earlyMinutes}分鐘</span></div>` : ''}
    ...
    <div className="page-container fade-in">
      {payslip && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.85)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={() => setPayslip(null)}>
          <div style={{background:'var(--black-card)',border:'1px solid var(--border-gold)',borderRadius:20,padding:24,width:'100%',maxWidth:440,maxHeight:'90vh',overflowY:'auto'}} onClick={e => e.stopPropagation()}>
            ...
            <div style={{fontSize:12,color:'var(--text-muted)',marginBottom:12}}>{payslip.month}{isCurrentMonth ? ` (截至 ${todayDay} 日)` : ''}</div>
      ...
      {isCurrentMonth && (
        <div style={{fontSize:11,color:'var(--gold)',background:'var(--gold-glow)',padding:'6px 12px',borderRadius:8,marginBottom:10,display:'flex',alignItems:'center',gap:4,border:'1px solid var(--border-gold)'}}>
          <Clock size={12}/> 即時計算：截至 {todayDay} 日（每日自動更新）
        </div>
      )}
      ...
      {tab === 'payroll' && (<div>
        ...
        {emps.map(emp => {
          ...
          // ✅ 組合出勤摘要（遲到、早退顯眼標示）
          const badges = []
          badges.push(`出勤${p.att.work}天`)
          if (p.att.lateCount) badges.push(`遲到${p.att.lateCount}`)
          if (p.att.earlyCount) badges.push(`早退${p.att.earlyCount}`)
          if (p.att.otDetails.length) badges.push(`加班${p.att.otDetails.length}次`)
          if (p.att.sick) badges.push(`病假${p.att.sick}`)
          if (p.att.personal) badges.push(`事假${p.att.personal}`)
          ...
          return <div key={emp.id} className="card" style={{marginBottom:8,padding:0,overflow:'hidden'}}>
            ...
            {ex && <div style={{padding:'0 14px 14px',borderTop:'1px solid var(--border)'}}>
              ...
              {/* ✅ 遲到明細 */}
              {p.att.lateDetails.length>0&&(<div style={{marginBottom:8}}>
                <div style={{fontSize:11,color:'var(--red)',fontWeight:600,marginBottom:4}}>⚠️ 遲到明細（寬限{LATE_GRACE_MIN}分）</div>
                {p.att.lateDetails.map((d,i) => <div key={i} style={{fontSize:11,color:'var(--text-dim)',display:'flex',justifyContent:'space-between',padding:'2px 0'}}><span>{d.date} · 打卡 {d.time}</span><span style={{color:'var(--red)'}}>遲{d.minutes}分</span></div>)}
              </div>)}

              {/* ✅ 早退明細 */}
              {p.att.earlyDetails.length>0&&(<div style={{marginBottom:8}}>
                <div style={{fontSize:11,color:'#f59e0b',fontWeight:600,marginBottom:4}}>⚠️ 早退明細</div>
                {p.att.earlyDetails.map((d,i) => <div key={i} style={{fontSize:11,color:'var(--text-dim)',display:'flex',justifyContent:'space-between',padding:'2px 0'}}><span>{d.date} · 下班 {d.time}</span><span style={{color:'#f59e0b'}}>早{d.minutes}分</span></div>)}
              </div>)}
