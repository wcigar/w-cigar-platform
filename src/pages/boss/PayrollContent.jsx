    ...
    <div className="page-container fade-in">
      ...
      {tab === 'expenses' && (<div>
        ...
        {showExpForm&&<div className="card" style={{marginBottom:16,padding:16}}>
          ...
          <div style={{display:'flex',gap:8,marginBottom:8}}><input type="number" value={newExp.amount} onChange={e => setNewExp(p=>({...p,amount:e.target.value}))} placeholder="金額" style={{flex:1}} inputMode="numeric"/><select value={newExp.payment} onChange={e => setNewExp(p=>({...p,payment:e.target.value}))} style={{width:100,fontSize:13,padding:8}}>{['現金','刷卡','轉帳','LINE Pay'].map(p => <option key={p}>{p}</option>)}</select></div>
