    ...
    <div className="page-container fade-in">
        ...
        {emps.map(emp => {
          ...
          return <div key={emp.id} className="card" style={{padding:14,marginBottom:8}}>
            ...
            {isE&&<div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
              ...
              <input type="tel" inputMode="numeric" pattern="[0-9]*" value={editingSal.monthly_salary} onChange={e => setEditingSal(p=>({...p,monthly_salary:e.target.value}))} style={{flex:1,fontSize:13,padding:8}}/>
      ...
      {tab === 'bonus' && (<div>
        ...
        {showBonusForm&&<div className="card" style={{marginBottom:16,padding:16}}>
          ...
          <input type="tel" inputMode="numeric" pattern="[0-9]*" value={newBonus.amount} onChange={e => setNewBonus(p=>({...p,amount:e.target.value}))} placeholder="金額" style={{marginBottom:8}} inputMode="numeric" pattern="[0-9]*"/>
      ...
      {tab === 'expenses' && (<div>
        ...
        {showExpForm&&<div className="card" style={{marginBottom:16,padding:16}}>
          ...
          <div style={{display:'flex',gap:8,marginBottom:8}}><input type="tel" inputMode="numeric" pattern="[0-9]*" value={newExp.amount} onChange={e => setNewExp(p=>({...p,amount:e.target.value}))} placeholder="金額" style={{flex:1}} inputMode="numeric" pattern="[0-9]*"/><select value={newExp.payment} onChange={e => setNewExp(p=>({...p,payment:e.target.value}))} style={{width:100,fontSize:13,padding:8}}>{['現金','刷卡','轉帳','LINE Pay'].map(p => <option key={p}>{p}</option>)}</select></div>
