...
import { Briefcase, Users, DollarSign, Settings, Clock, AlertTriangle, CheckCircle2 } from 'lucide-react'
import { format } from 'date-fns'
import { zhTW } from 'date-fns/locale'

export default function BossHome() {
  const navigate = useNavigate()
  const [stats, setStats] = useState({ employees: 0, todayWorking: 0, sopCompletion: 0 })
  ...
  async function loadDashboard() {
    setLoading(true)
    const [empRes, schedRes, taskRes] = await Promise.all([
    ...
    const working = schedules.filter(s => s.shift !== '休假')
    setStats({
  ...
  return (
    <div className="page-container fade-in">
      ...
      <div className="grid-2" style={{ marginBottom: 20 }}>
        ...
        <StatBox icon={<AlertTriangle size={16}/>} label="排班人數" value={todaySchedules.length} color="var(--red)" />
      </div>
      ...
      {todaySchedules.map(s => (
        <div key={s.id} className="card" style={{ padding: 12, marginBottom: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span className={`badge ${s.shift === '休假' ? 'badge-blue' : 'badge-gold'}`}>{s.shift}</span>
        </div>
      ))}
      {todaySchedules.length === 0 && <div style={{ fontSize: 13, color: 'var(--text-dim)', padding: 16, textAlign: 'center' }}>今日無排班資料</div>}
    </div>
  )
}
