      ...
      {editing && (
        <div className="card" style={{ padding: 16, marginBottom: 14, borderColor: 'var(--border-gold)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
            <div><div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 2 }}>\ud83d\udcb5 \u73fe\u91d1</div><input type="tel" inputMode="numeric" pattern="[0-9]*" value={form.cash} onChange={e => setForm(p => ({...p, cash: e.target.value}))} placeholder="0" style={{ fontSize: 16, padding: 8, fontFamily: 'var(--font-mono)' }} /></div>
            <div><div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 2 }}>\ud83d\udcb3 \u5237\u5361</div><input type="tel" inputMode="numeric" pattern="[0-9]*" value={form.card} onChange={e => setForm(p => ({...p, card: e.target.value}))} placeholder="0" style={{ fontSize: 16, padding: 8, fontFamily: 'var(--font-mono)' }} /></div>
            <div><div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 2 }}>\ud83c\udfe6 \u8f49\u5e33</div><input type="tel" inputMode="numeric" pattern="[0-9]*" value={form.transfer} onChange={e => setForm(p => ({...p, transfer: e.target.value}))} placeholder="0" style={{ fontSize: 16, padding: 8, fontFamily: 'var(--font-mono)' }} /></div>
            <div><div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 2 }}>\ud83d\udc65 \u4eba\u6578</div><input type="tel" inputMode="numeric" pattern="[0-9]*" value={form.customers} onChange={e => setForm(p => ({...p, customers: e.target.value}))} placeholder="0" style={{ fontSize: 16, padding: 8, fontFamily: 'var(--font-mono)' }} /></div>
          </div>
