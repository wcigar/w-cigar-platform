...
export default function PosLogin({ onLogin }) {
  ...
  useEffect(() => {
    supabase.rpc('list_pos_staff').then(({ data, error: err }) => {
      if (err) { console.error('list_pos_staff failed:', err); return }
      setEmployees(data || [])
    })
  }, [])
  ...
  async function doLogin(empId, pinCode) {
    ...
    setError('')
    try {
      const { data, error: err } = await supabase.rpc('staff_login', { p_code: pinCode })
      if (err) throw new Error(err.message)
      if (!data?.success) throw new Error(data?.error || 'PIN 碼錯誤')
      if (data.id !== empId) throw new Error('PIN 碼錯誤')
      if (!data.can_use_pos) throw new Error('此帳號未開放 POS 使用權限')
