/**
 * POS Checkout â ä¸»çµå¸³é é¢
 * - å¾ StaffPOS æ½åæ ¸å¿éè¼¯
 * - ä½¿ç¨ pos_checkout_v2 RPCï¼å«æ¹å¹é©è­ï¼
 * - Fallback å° pos_checkoutï¼v2 å°æªå»ºç«æï¼
 * - iPad æ©«å±åªå
 */
...
export default function PosCheckout({ session, shift, onShiftChange, onCartCountChange, onHeldCountChange, showHeldFromLayout, onHeldFromLayoutDone, showOrdersFromLayout, onOrdersFromLayoutDone }) {
  ...
  const [attributedTo, setAttributedTo] = useState('åºå§')
  ...
  const [newCust, setNewCust] = useState({ name: '', phone: '', belongs_to: 'åºå§', notes: '' })
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(rpc) && rpc.length > 0 && rpc[0].name) {
        cigars = rpc.map(p => ({ ...p, _src: 'cigar', _cat: p.brand === 'Capadura' ? 'Capadura' : (SECTION_CATEGORY_MAP[p.sections?.[0]] || classifyItem(p.name, p.category) || 'å¤å·´éªè'), _price: p.suggest_price || p.price_a || 0, _stock: p.stock_status || 'ç¾è²¨' }))
      } else {
        ...
        cigars = (dp || []).map(p => ({ ...p, _src: 'cigar', _cat: p.brand === 'Capadura' ? 'Capadura' : (SECTION_CATEGORY_MAP[p.sections?.[0]] || classifyItem(p.name, p.category) || 'å¤å·´éªè'), _price: p.suggest_price || p.price_a || 0, _stock: p.stock_status || 'ç¾è²¨' }))
      }
      const { data: inv } = await supabase.from('inventory_master').select('id, name, category, current_stock, safe_stock, retail_price, image_url').eq('enabled', true).in('category', ['å§å°é£²å', 'é¤é£²', 'éé¡', 'éä»¶', 'çéèæ']).gt('retail_price', 0)
  ...
  async function holdCurrentOrder() {
    if (!cart.length) return alert('è³¼ç©è»æ¯ç©ºç')
    ...
    clearAll(); loadHeldOrders(); alert('â å·²æå®æ«å­')
  }
  ...
  async function resumeHeldOrder(held) {
    ...
    setAttributedTo(held.attributed_to || 'åºå§'); setDiscountPct(held.discount_pct || 0); setServiceFeePct(held.service_fee_pct || 0); setOrderNote(held.notes || '')
  ...
  async function cancelHeldOrder(held) { if (!confirm('ç¢ºå®åæ¶æ­¤æå®ï¼')) return; await supabase.from('pos_held_orders').update({ status: 'cancelled' }).eq('id', held.id); loadHeldOrders() }

  // ââ Today Orders ââ
  ...
  async function doVoidOrder() {
    if (!voidingOrder || !voidReason.trim()) return alert('è«è¼¸å¥ä½å»¢åå ')
    ...
    if (error || !data?.success) return alert('ä½å»¢å¤±æ: ' + (data?.error || error?.message))
    alert('â ' + data.message); setVoidingOrder(null); setVoidReason(''); loadTodayOrders(); loadAll()
  }
  ...
  function selectCustomer(c) { setCustomer(c); setCustomerTier(tiers.find(t => t.id === c.membership_tier) || null); setAttributedTo(c.belongs_to || 'åºå§'); setShowCustomerSearch(false) }
  function clearCustomer() { setCustomer(null); setCustomerTier(null); setAttributedTo('åºå§') }
  ...
  function addToCart(p, qty, note) {
    if (p._stock === 'ç¼ºè²¨' || p._price <= 0) return
    setCart(prev => {
  ...
  function openDetail(p) { if (p._stock === 'ç¼ºè²¨' || p._price <= 0) return; setDetailProduct(p); setDetailQty(1); setDetailNote('') }
  ...
  function applyPriceOverride(id) {
    ...
    logAudit('price_override', `[POS-APP] ${item.name}: $${item._originalPrice || item.price} â $${newPrice}${editPriceReason ? ' åå : ' + editPriceReason : ''}`, session?.name || 'UNKNOWN')
  ...
  function clearAll() {
    ...
    setTaxId(''); setCarrier(''); setOrderNote(''); clearCustomer(); setAttributedTo('åºå§')
  }
  ...
  // ââ Checkout (try v2, fallback to v1) ââ
  async function doCheckout() {
    if (!cart.length) return
    if (payMethod === 'cash' && cashPaid < total) return alert('ç¾éä¸è¶³')
    setSubmitting(true)
    try {
      ...
      const checkoutParams = {
        ...
        p_items: cart.map(c => ({
          ...
          brand: c.brand || '',
          category: c.category ||
            (c.sections?.[0] ? (SECTION_CATEGORY_MAP[c.sections[0]] || c.sections[0]) : 'å¶ä»'),
        ...
        p_store_id: STORE_ID,
        p_notes: [
          customer ? `å®¢æ¶: ${customer.name}` : '',
          guestCount > 1 ? `äººæ¸: ${guestCount}` : '',
          serviceFeePct > 0 ? `æåè²»: ${serviceFeePct}%` : '',
          invoiceEnabled ? `çµ±ç·¨: ${taxId} è¼å·: ${carrier}` : '',
          orderNote,
          ...cart.filter(c => c._overridden).map(c => `[æ¹å¹] ${c.name}: $${c._originalPrice}â$${c.price}`),
      ...
      if (error || !data?.success) throw new Error(error?.message || data?.error || 'çµå¸³å¤±æ')

      try { await supabase.from('unified_orders').update({ attributed_to: customer ? (customer.belongs_to || 'åºå§') : attributedTo }).eq('order_no', data.order_no) } catch {}
    ...
    } catch (e) { alert('çµå¸³å¤±æ: ' + e.message) } finally { setSubmitting(false) }
  }
  ...
  async function closeShiftFn() {
    ...
    if (data?.success) { alert('éç­å®æï¼å·®é¡: $' + (data.variance ?? 0)); setShowShift(false); setClosingCash(''); onShiftChange(null); loadAll() }
    else alert('éç­å¤±æ: ' + (data?.error || ''))
  }
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  <div class="wrap">
    <div class="left">
      <div>
        <div class="brand">W CIGAR BAR ç´³å£«éªèé¤¨</div>
        <div class="customer">${customer?.name || 'è¨ªå®¢'}</div>
        <div class="cigar">${item.name}</div>
      </div>
      <div>
        <div class="cabinet">çªä½ï¼${customer?.cabinet_no || 'â'}</div>
        <div class="date">å¥åº«ï¼${now} ï½ ${orderNo.slice(-8)}</div>
      </div>
    </div>
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  const ts = customer ? (TIER_STYLES[customer.membership_tier] || TIER_STYLES['éæå¡']) : null
  const gridCols = isMobile ? 'repeat(2, 1fr)' : isTablet ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)'

  // ââ Checkout bottom panel ââ
  function CheckoutBottom() {
    ...
    return <>
      <div style={{ display: 'flex', gap: 6, marginBottom: 4, alignItems: 'center' }}>
        <div style={{ flex: 1 }}><span style={{ fontSize: 8, color: '#8a7e6e' }}>ææ£%</span><input type="number" min={0} max={100} value={discountPct || ''} onChange={e => setDiscountPct(Math.min(100, Math.max(0, +e.target.value || 0)))} placeholder="0" style={{ ...is, height: 28, fontFamily: 'var(--font-mono)' }} /></div>
        <div style={{ flex: 1 }}><span style={{ fontSize: 8, color: '#8a7e6e' }}>æåè²»%</span><input type="number" min={0} max={100} value={serviceFeePct || ''} onChange={e => setServiceFeePct(Math.min(100, Math.max(0, +e.target.value || 0)))} placeholder="0" style={{ ...is, height: 28, fontFamily: 'var(--font-mono)' }} /></div>
        <label style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 10, color: '#8a7e6e', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}><input type="checkbox" checked={invoiceEnabled} onChange={e => setInvoiceEnabled(e.target.checked)} /> éç¼ç¥¨</label>
      </div>
      {invoiceEnabled && <div style={{ display: 'flex', gap: 4, marginBottom: 3 }}><input value={taxId} onChange={e => setTaxId(e.target.value)} placeholder="çµ±ä¸ç·¨è" style={{ ...is, flex: 1, height: 28 }} /><input value={carrier} onChange={e => setCarrier(e.target.value)} placeholder="è¼å·" style={{ ...is, flex: 1, height: 28 }} /></div>}
      <input value={orderNote} onChange={e => setOrderNote(e.target.value)} placeholder="åè¨»â¦" style={{ ...is, width: '100%', height: 28, marginBottom: 3 }} />
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#8a7e6e' }}><span>å± {cartCount} ä»¶</span><span>${subtotal.toLocaleString()}</span></div>
      {memberDiscount.discount > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#9b59b6' }}><span>æå¡ææ£</span><span>-${memberDiscount.discount.toLocaleString()}</span></div>}
      {manualDiscountAmt > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#f59e0b' }}><span>ææ£ {discountPct}%</span><span>-${manualDiscountAmt.toLocaleString()}</span></div>}
      {serviceFeeAmt > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#8a7e6e' }}><span>æåè²»</span><span>+${serviceFeeAmt.toLocaleString()}</span></div>}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', margin: '2px 0' }}><span style={{ fontSize: 16, fontWeight: 800, color: '#c9a84c' }}>ææ¶</span><span style={{ fontSize: 24, fontWeight: 800, color: '#c9a84c', fontFamily: 'var(--font-mono)' }}>${total.toLocaleString()}</span></div>
      <button onClick={() => { if (cart.length) setShowCheckout(true) }} disabled={!cart.length} style={{ width: '100%', padding: isTablet ? '14px 0' : '10px 0', fontSize: isTablet ? 18 : 16, fontWeight: 700, cursor: cart.length ? 'pointer' : 'not-allowed', background: cart.length ? '#c9a84c' : '#2a2520', border: 'none', borderRadius: 8, color: cart.length ? '#0d0b09' : '#8a7e6e', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 4 }}><CreditCard size={16} /> çµå¸³ ${total.toLocaleString()}</button>
      <div style={{ display: 'flex', gap: 6 }}>
        <button onClick={holdCurrentOrder} style={{ flex: 1, padding: '6px 0', fontSize: 11, background: '#1a1714', color: '#c9a84c', border: '1px solid #2a2520', borderRadius: 6, cursor: 'pointer' }}>æ«å­</button>
        <button onClick={() => { loadHeldOrders(); setShowHeldModal(true) }} style={{ flex: 1, padding: '6px 0', fontSize: 11, background: '#1a1714', color: heldOrders.length ? '#f59e0b' : '#c9a84c', border: '1px solid #2a2520', borderRadius: 6, cursor: 'pointer', position: 'relative' }}>æå®{heldOrders.length > 0 && <span style={{ position: 'absolute', top: -6, right: -4, background: '#e74c3c', color: '#fff', borderRadius: '50%', width: 16, height: 16, fontSize: 9, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{heldOrders.length}</span>}</button>
        <button onClick={clearAll} style={{ flex: 1, padding: '6px 0', fontSize: 11, background: '#1a1714', color: '#e74c3c', border: '1px solid #2a2520', borderRadius: 6, cursor: 'pointer' }}>æ¸é¤</button>
      </div>
    </>
  }
      ...
      {cart.map(c => (
        <div key={c.id} style={{ padding: '6px 0', borderBottom: '1px solid #2a2520' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10, color: '#8a7e6e', fontFamily: 'var(--font-mono)', display: 'flex', alignItems: 'center', gap: 3 }}>
                {editPriceId === c.id ? null : <>
                  ...
                  {c._overridden && <span style={{ fontSize: 8, color: '#f59e0b', fontWeight: 600 }}>æ¹</span>}
                  <span> Ã {c.qty}{c.note ? ` Â· ${c.note}` : ''}</span>
                </>}
              </div>
            </div>
            ...
            {!['å¥¶è¶åå¡','æ°£æ³¡é£²å','éé¡','é¤é£','çé»'].includes(c._cat) && (
          ...
          {editPriceId === c.id && (
            <div style={{ display: 'flex', gap: 4, marginTop: 4, alignItems: 'center' }}>
              ...
              <input placeholder="åå " value={editPriceReason} onChange={e => setEditPriceReason(e.target.value)}
              ...
              <button onClick={() => applyPriceOverride(c.id)} style={{ fontSize: 10, padding: '3px 8px', background: '#f59e0b', color: '#000', border: 'none', borderRadius: 4, cursor: 'pointer', fontWeight: 700, flexShrink: 0 }}>â</button>
              <button onClick={() => setEditPriceId(null)} style={{ fontSize: 10, padding: '3px 6px', background: 'none', color: '#8a7e6e', border: '1px solid #2a2520', borderRadius: 4, cursor: 'pointer', flexShrink: 0 }}>â</button>
            </div>
          )}
        </div>
      ))}
      {memberDiscount.details.length > 0 && <div style={{ padding: '6px 0', borderBottom: '1px solid #2a2520' }}>
        {memberDiscount.details.map((d, i) => <div key={i} style={{ fontSize: 10, color: '#9b59b6', display: 'flex', justifyContent: 'space-between' }}><span>{d.name} Â· {d.rate === 0 ? 'åè²»' : `${Math.round(d.rate * 10)}æ`}</span><span>-${d.saved.toLocaleString()}</span></div>)}
      </div>}
    </>
  }
  ...
  function PCard({ p }) {
    ...
    const isOut = p._stock === 'ç¼ºè²¨'
    const isLow = p._stock === 'å°é'
    return (
      <button onClick={() => openDetail(p)} disabled={isOut}
        ...
        {isOut && <span style={{ position: 'absolute', top: 4, right: 4, zIndex: 2, background: 'rgba(0,0,0,.75)', color: '#e74c3c', borderRadius: 6, padding: '1px 6px', fontSize: 10, fontWeight: 700 }}>å®ç½</span>}
        ...
        <div style={{ padding: '6px 8px' }}>
          ...
          {isLow && !isOut && <div style={{ fontSize: 9, color: '#f59e0b', fontWeight: 600, marginTop: 2 }}>åº«å­å°</div>}
        </div>
      </button>
    )
  }
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#0d0b09', color: '#e8dcc8' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderBottom: '1px solid #2a2520', flexShrink: 0 }}>
        <button onClick={() => setShowShift(true)} style={{ background: 'none', border: '1px solid #2a2520', borderRadius: 6, padding: '3px 8px', fontSize: 10, color: '#8a7e6e', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3, flexShrink: 0 }}><Clock size={11} /> {shift ? 'éç­' : 'éç­'}</button>
        <div style={{ position: 'relative', flex: 1, minWidth: 80 }}>
          <Search size={13} style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', color: '#8a7e6e' }} />
          <input placeholder="æå°åç / åå / SKUâ¦" value={search} onChange={e => setSearch(e.target.value)}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: isMobile ? 1 : isTablet ? '0 0 55%' : '0 0 65%', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: isMobile ? 'none' : '1px solid #2a2520' }}>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: 8 }}>
            ...
            {!filtered.length && <div style={{ textAlign: 'center', padding: 40, color: '#8a7e6e' }}>ç¡ç¬¦ååå</div>}
          </div>
          <div style={{ padding: '4px 8px', borderTop: '1px solid #2a2520', display: 'flex', gap: 3, justifyContent: 'center', flexShrink: 0 }}>
            {QTY_PRESETS.map(n => <button key={n} onClick={() => setDetailQty(n)} style={{ padding: isTablet ? '8px 16px' : '3px 12px', borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: 'pointer', background: detailQty === n ? 'rgba(201,168,76,.15)' : '#0d0b09', color: detailQty === n ? '#c9a84c' : '#8a7e6e', border: detailQty === n ? '1px solid rgba(201,168,76,.3)' : '1px solid #2a2520' }}>Ã{n}</button>)}
          </div>
        </div>
            ...
            <div style={{ padding: '8px 12px', borderBottom: '1px solid #2a2520', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 4 }}>
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                ...
                <select value={tableNo} onChange={e => setTableNo(e.target.value)} style={{ fontSize: 11, padding: '4px', background: '#1a1714', border: '1px solid #2a2520', borderRadius: 6, color: '#e8dcc8' }}>
                  <option value="">æ¡ä½</option>{TABLES.map(t => <option key={t}>{t}</option>)}
                </select>
                ...
                {cart.length > 0 && <button onClick={clearAll} style={{ background: 'none', border: 'none', color: '#e74c3c', fontSize: 10, cursor: 'pointer' }}>æ¸ç©º</button>}
              </div>
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                {!customer ? <>
                  <button style={{ flex: 1, padding: '4px 0', borderRadius: 6, fontSize: 10, fontWeight: 700, background: 'rgba(201,168,76,.15)', color: '#c9a84c', border: '1px solid rgba(201,168,76,.3)', cursor: 'default' }}>æ£å®¢</button>
                  <button onClick={() => { setShowCustomerSearch(true); setCustomerQuery('') }} style={{ flex: 1, padding: '4px 0', borderRadius: 6, fontSize: 10, fontWeight: 700, cursor: 'pointer', background: '#1a1714', color: '#8a7e6e', border: '1px solid #2a2520', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}><User size={11} /> é¸åæå¡</button>
              ...
              {!customer && <div style={{ display: 'flex', gap: 3, alignItems: 'center' }}>
                <span style={{ fontSize: 9, color: '#8a7e6e', whiteSpace: 'nowrap' }}>æ­¸å±¬</span>
                {['èé', 'èéå¨', 'åºå§'].map(a => <button key={a} onClick={() => setAttributedTo(a)} style={{ flex: 1, padding: '2px 0', borderRadius: 6, fontSize: 9, fontWeight: 600, cursor: 'pointer', background: attributedTo === a ? 'rgba(201,168,76,.15)' : '#1a1714', color: attributedTo === a ? '#c9a84c' : '#8a7e6e', border: attributedTo === a ? '1px solid rgba(201,168,76,.3)' : '1px solid #2a2520' }}>{a}</button>)}
              </div>}
            </div>
            {/* Cart items (scrollable) */}
            <div style={{ flex: '1 1 0', overflowY: 'auto', padding: '4px 12px', minHeight: 0, WebkitOverflowScrolling: 'touch' }}>
              {!cart.length ? <div style={{ textAlign: 'center', padding: 40, color: '#8a7e6e', fontSize: 13 }}>é»é¸ååå å¥è³¼ç©è»</div> : <CartItems />}
            </div>
      ...
      {isMobile && !showMobileCart && (
        <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', paddingBottom: 'max(10px, env(safe-area-inset-bottom))', borderTop: '2px solid rgba(201,168,76,.4)', background: '#1a1714' }} onClick={() => setShowMobileCart(true)}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><ShoppingCart size={16} color="#c9a84c" /><span style={{ fontWeight: 700 }}>{cartCount} ä»¶</span><span style={{ color: '#c9a84c', fontWeight: 800, fontSize: 18 }}>${total.toLocaleString()}</span></div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }} onClick={() => setShowMobileCart(false)}>
          <div style={{ background: '#1a1714', borderRadius: '16px 16px 0 0', maxHeight: '85vh', display: 'flex', flexDirection: 'column', border: '1px solid rgba(201,168,76,.3)', borderBottom: 'none' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #2a2520', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <span style={{ fontSize: 16, fontWeight: 700, color: '#c9a84c' }}>è³¼ç©è» ({cartCount})</span>
              <button onClick={() => setShowMobileCart(false)} style={{ background: 'none', border: 'none', color: '#8a7e6e', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}><ChevronDown size={18} /> æ¶èµ·</button>
            </div>
            <div style={{ flex: '1 1 0', overflowY: 'auto', padding: '4px 16px', minHeight: 0 }}>
              {!cart.length ? <div style={{ textAlign: 'center', padding: 24, color: '#8a7e6e' }}>è³¼ç©è»æ¯ç©ºç</div> : <CartItems />}
            </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setDetailProduct(null)}>
          <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, width: '100%', maxWidth: 400, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '16px 20px' }}>
              ...
              <input value={detailNote} onChange={e => setDetailNote(e.target.value)} placeholder="åè¨»ï¼å»å°ãå°ç³...ï¼" style={{ width: '100%', fontSize: 12, padding: '8px 10px', background: '#0d0b09', border: '1px solid #2a2520', borderRadius: 8, color: '#e8dcc8', marginBottom: 14, boxSizing: 'border-box' }} />
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setDetailProduct(null)} style={{ flex: 1, padding: 12, borderRadius: 10, border: '1px solid #2a2520', background: '#0d0b09', color: '#8a7e6e', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>åæ¶</button>
                <button onClick={() => { addToCart(detailProduct, detailQty, detailNote); setDetailProduct(null) }} style={{ flex: 2, padding: 12, borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #c9a84c, #b8943f)', color: '#000', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}><ShoppingCart size={16} /> å å¥ Â· ${(detailProduct._price * detailQty).toLocaleString()}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showCheckout && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setShowCheckout(false)}>
          <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}><span style={{ fontSize: 18, fontWeight: 700, color: '#c9a84c' }}>ð° çµå¸³</span><button onClick={() => setShowCheckout(false)} style={{ background: 'none', border: 'none', color: '#8a7e6e', cursor: 'pointer' }}><X size={20} /></button></div>
            <div style={{ background: '#0d0b09', borderRadius: 12, padding: 12, marginBottom: 14 }}>
              {cart.map(c => <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '3px 0', color: '#e8dcc8' }}><span>{c.name} Ã{c.qty}{c._overridden ? ' â¡' : ''}</span><span style={{ fontFamily: 'var(--font-mono)', fontWeight: 600 }}>${(c.price * c.qty).toLocaleString()}</span></div>)}
              {memberDiscount.discount > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '3px 0', color: '#9b59b6' }}><span>æå¡ææ£</span><span>-${memberDiscount.discount.toLocaleString()}</span></div>}
              {manualDiscountAmt > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '3px 0', color: '#f59e0b' }}><span>æåææ£</span><span>-${manualDiscountAmt.toLocaleString()}</span></div>}
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 18, fontWeight: 800, color: '#c9a84c', borderTop: '1px solid rgba(201,168,76,.3)', marginTop: 6, paddingTop: 8 }}><span>ææ¶</span><span>${total.toLocaleString()}</span></div>
            </div>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#8a7e6e', marginBottom: 6 }}>æ¯ä»æ¹å¼</div>
            ...
            {payMethod === 'cash' && <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#8a7e6e', marginBottom: 6 }}>æ¶åéé¡</div>
              ...
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                ...
                <button onClick={() => setPayAmount(String(total))} style={{ flex: 1, minWidth: 50, padding: '8px 4px', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: 'rgba(77,168,108,.1)', color: '#4da86c', border: '1px solid rgba(77,168,108,.3)' }}>åå¥½</button>
              </div>
              {+payAmount >= total && <div style={{ marginTop: 10, textAlign: 'center', padding: 12, background: 'rgba(77,168,108,.08)', borderRadius: 12, border: '1px solid rgba(77,168,108,.3)' }}><div style={{ fontSize: 12, color: '#8a7e6e' }}>æ¾é¶</div><div style={{ fontSize: 32, fontFamily: 'var(--font-mono)', fontWeight: 800, color: '#4da86c' }}>${change.toLocaleString()}</div></div>}
            </div>}
            <button onClick={doCheckout} disabled={submitting || (payMethod === 'cash' && cashPaid < total)} style={{ width: '100%', padding: 16, fontSize: 18, fontWeight: 700, cursor: 'pointer', background: 'linear-gradient(135deg, #4da86c, #2d8a4e)', border: 'none', borderRadius: 14, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: submitting ? .5 : 1 }}><CheckCircle2 size={20} /> {submitting ? 'èçä¸­...' : 'ç¢ºèªçµå¸³'}</button>
          </div>
        </div>
      )}
      ...
      {lastOrder && (
        <div id="receipt-print" style={{ display: 'none' }}>
          <div style={{ textAlign: 'center', borderBottom: '1px dashed #000', paddingBottom: 6, marginBottom: 6 }}>
            ...
            <div style={{ fontSize: 10 }}>ç´³å£«éªèé¤¨ ï½ å°åå¸å¤§å®å</div>
          </div>
          <div style={{ marginBottom: 6, fontSize: 11 }}>
            <div>{new Date().toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' })}</div>
            <div>å®èï¼{lastOrder.order_no}</div>
            <div>æ¶éï¼{session?.name}</div>
            {customer && <div>å®¢æ¶ï¼{customer.name}</div>}
          </div>
          <div style={{ borderTop: '1px dashed #000', borderBottom: '1px dashed #000', padding: '4px 0', marginBottom: 6 }}>
            {cart.map((item, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                <span style={{ flex: 1 }}>{item.name} Ã{item.qty}</span>
                <span>${(item.price * item.qty).toLocaleString()}</span>
              </div>
            ))}
          </div>
          <div style={{ marginBottom: 6 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>å°è¨</span><span>${subtotal.toLocaleString()}</span></div>
            {memberDiscount.discount > 0 && <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>æå¡ææ£</span><span>-${memberDiscount.discount.toLocaleString()}</span></div>}
            {manualDiscountAmt > 0 && <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>ææ£ {discountPct}%</span><span>-${manualDiscountAmt.toLocaleString()}</span></div>}
            {serviceFeeAmt > 0 && <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>æåè²»</span><span>+${serviceFeeAmt.toLocaleString()}</span></div>}
            <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', fontSize: 13, marginTop: 2 }}><span>ç¸½è¨</span><span>${lastOrder.total?.toLocaleString()}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>å¯¦æ¶</span><span>${lastOrder.paid?.toLocaleString()}</span></div>
            {lastOrder.change > 0 && <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>æ¾é¶</span><span>${lastOrder.change?.toLocaleString()}</span></div>}
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>ä»æ¬¾</span><span>{payMethod}</span></div>
          </div>
          <div style={{ textAlign: 'center', borderTop: '1px dashed #000', paddingTop: 6, fontSize: 10 }}>
            <div>æè¬æ¨èè¨ W Cigar Bar</div>
            <div>è«ä¿çæ¬æ¶æä»¥ä¾æ¥è©¢</div>
          </div>
        </div>
      )}
      ...
      {lastOrder && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => { setLastOrder(null); setUpgradeInfo(null) }}>
          <div style={{ background: '#1a1714', border: '2px solid rgba(77,168,108,.5)', borderRadius: 20, padding: 30, width: '100%', maxWidth: 400, textAlign: 'center' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: '#4da86c', marginBottom: 4 }}>çµå¸³æåï¼</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: lastOrder.change > 0 ? '1fr 1fr 1fr' : '1fr 1fr', gap: 8, marginBottom: 12, background: '#0d0b09', borderRadius: 12, padding: 12 }}>
              <div><div style={{ fontSize: 10, color: '#8a7e6e' }}>ææ¶</div><div style={{ fontSize: 18, fontFamily: 'var(--font-mono)', fontWeight: 700, color: '#c9a84c' }}>${lastOrder.total?.toLocaleString()}</div></div>
              <div><div style={{ fontSize: 10, color: '#8a7e6e' }}>å¯¦æ¶</div><div style={{ fontSize: 18, fontFamily: 'var(--font-mono)', fontWeight: 700, color: '#e8dcc8' }}>${lastOrder.paid?.toLocaleString()}</div></div>
              {lastOrder.change > 0 && <div><div style={{ fontSize: 10, color: '#8a7e6e' }}>æ¾é¶</div><div style={{ fontSize: 18, fontFamily: 'var(--font-mono)', fontWeight: 700, color: '#4da86c' }}>${lastOrder.change?.toLocaleString()}</div></div>}
            </div>
            {upgradeInfo && <div style={{ background: 'rgba(155,89,182,.1)', border: '1px solid rgba(155,89,182,.3)', borderRadius: 12, padding: 12, marginBottom: 12 }}><div style={{ fontSize: 16, fontWeight: 700, color: '#9b59b6' }}>ð åç´ï¼</div><div style={{ fontSize: 13, color: '#e8dcc8', marginTop: 4 }}>{lastOrder.customerName} â <b style={{ color: '#c9a84c' }}>ã{upgradeInfo.new_tier}ã</b></div></div>}
            <button onClick={printReceipt} style={{ padding: '10px 20px', borderRadius: 10, border: 'none', background: '#c9a84c', color: '#1a1410', fontSize: 13, fontWeight: 700, cursor: 'pointer', marginBottom: 8, width: '100%' }}>ð¨ï¸ åå°æ¶æ</button>
            <button onClick={() => { setLastOrder(null); setUpgradeInfo(null) }} style={{ padding: '12px 40px', fontSize: 16, fontWeight: 700, cursor: 'pointer', background: '#c9a84c', border: 'none', borderRadius: 12, color: '#000' }}>å®æ</button>
          </div>
        </div>
      )}
      ...
      {showCustomerSearch && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setShowCustomerSearch(false)}>
          <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, padding: 20, width: '100%', maxWidth: 440, maxHeight: '80vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}><span style={{ fontSize: 16, fontWeight: 700, color: '#c9a84c', display: 'flex', alignItems: 'center', gap: 6 }}><User size={18} /> é¸åå®¢æ¶</span><button onClick={() => setShowCustomerSearch(false)} style={{ background: 'none', border: 'none', color: '#8a7e6e', cursor: 'pointer' }}><X size={20} /></button></div>
            <div style={{ position: 'relative', marginBottom: 10 }}><Search size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#8a7e6e' }} /><input placeholder="æå°å§å / é»è©±â¦" value={customerQuery} onChange={e => { setCustomerQuery(e.target.value); searchCustomers(e.target.value) }} autoFocus style={{ width: '100%', fontSize: 13, padding: '8px 10px 8px 32px', background: '#0d0b09', border: '1px solid #2a2520', borderRadius: 10, color: '#e8dcc8', boxSizing: 'border-box' }} /></div>
            <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>{['all', 'å°æ¦®æå¡', 'é²éæå¡', 'ç´³å£«ä¿±æ¨é¨'].map(f => <button key={f} onClick={() => setCustomerFilter(f)} style={{ padding: '3px 10px', borderRadius: 10, fontSize: 10, fontWeight: 600, cursor: 'pointer', background: customerFilter === f ? 'rgba(201,168,76,.15)' : 'transparent', color: customerFilter === f ? '#c9a84c' : '#8a7e6e', border: customerFilter === f ? '1px solid rgba(201,168,76,.3)' : '1px solid transparent' }}>{f === 'all' ? 'å¨é¨' : (TIER_STYLES[f]?.label || f).slice(0, 4)}</button>)}</div>
            {/* Quick add */}
            <button onClick={() => setShowQuickAdd(!showQuickAdd)} style={{ width: '100%', padding: 8, marginBottom: 8, background: showQuickAdd ? '#2a2520' : 'transparent', color: '#c9a84c', border: '1px dashed #c9a84c', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>{showQuickAdd ? 'â¼ æ¶èµ·' : '+ å¿«éæ°å¢å®¢æ¶'}</button>
            {showQuickAdd && <div style={{ background: '#1a1714', borderRadius: 8, padding: 12, marginBottom: 12, border: '1px solid #2a2520' }}>
              <input placeholder="å§å *" value={newCust.name} onChange={e => setNewCust({ ...newCust, name: e.target.value })} style={{ width: '100%', marginBottom: 8, padding: 8, background: '#0d0b09', color: '#e8dcc8', border: '1px solid #2a2520', borderRadius: 6, boxSizing: 'border-box' }} />
              <input placeholder="é»è©±" value={newCust.phone} onChange={e => setNewCust({ ...newCust, phone: e.target.value })} style={{ width: '100%', marginBottom: 8, padding: 8, background: '#0d0b09', color: '#e8dcc8', border: '1px solid #2a2520', borderRadius: 6, boxSizing: 'border-box' }} />
              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                <span style={{ color: '#8a7e6e', fontSize: 12, lineHeight: '32px' }}>æ­¸å±¬ï¼</span>
                {['èé', 'èéå¨', 'åºå§'].map(v => <button key={v} onClick={() => setNewCust({ ...newCust, belongs_to: v })} style={{ flex: 1, padding: '6px 0', fontSize: 12, borderRadius: 6, background: newCust.belongs_to === v ? '#c9a84c' : '#0d0b09', color: newCust.belongs_to === v ? '#0d0b09' : '#e8dcc8', border: '1px solid #2a2520', cursor: 'pointer' }}>{v}</button>)}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setShowQuickAdd(false)} style={{ flex: 1, padding: 8, background: '#0d0b09', color: '#8a7e6e', border: '1px solid #2a2520', borderRadius: 6, cursor: 'pointer' }}>åæ¶</button>
                <button onClick={async () => {
                  if (!newCust.name.trim()) return alert('è«è¼¸å¥å§å')
                  const { data, error } = await supabase.from('customers').insert({ name: newCust.name.trim(), phone: newCust.phone.trim() || null, belongs_to: newCust.belongs_to, notes: newCust.notes?.trim() || null, customer_type: 'æå¡', membership_tier: 'éæå¡', total_spent: 0, enabled: true }).select().single()
                  if (error) return alert('æ°å¢å¤±æï¼' + error.message)
                  selectCustomer(data); setShowQuickAdd(false); setNewCust({ name: '', phone: '', belongs_to: 'åºå§', notes: '' })
                }} style={{ flex: 1, padding: 8, background: '#c9a84c', color: '#0d0b09', border: 'none', borderRadius: 6, fontWeight: 700, cursor: 'pointer' }}>å²å­ä¸¦é¸å</button>
              </div>
            </div>}
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {customerSearching ? <div style={{ textAlign: 'center', padding: 20, color: '#8a7e6e' }}>æå°ä¸­â¦</div> : customerResults.filter(c => customerFilter === 'all' || c.membership_tier === customerFilter).map(c => {
                const s = TIER_STYLES[c.membership_tier] || TIER_STYLES['éæå¡']
                return <button key={c.id} onClick={() => selectCustomer(c)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: '#0d0b09', border: `1px solid ${s.border}40`, borderRadius: 10, cursor: 'pointer', textAlign: 'left', marginBottom: 6 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 700, color: '#e8dcc8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</div><div style={{ fontSize: 10, color: '#8a7e6e' }}>{c.phone || 'â'}</div></div>
      ...
      {showHeldModal && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setShowHeldModal(false)}>
        <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, padding: 20, width: '100%', maxWidth: 500, maxHeight: '80vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexShrink: 0 }}><span style={{ fontSize: 16, fontWeight: 700, color: '#f59e0b' }}>â¸ æå®åè¡¨ ({heldOrders.length})</span><button onClick={() => setShowHeldModal(false)} style={{ background: 'none', border: 'none', color: '#8a7e6e', cursor: 'pointer' }}><X size={20} /></button></div>
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {!heldOrders.length ? <div style={{ textAlign: 'center', padding: 40, color: '#8a7e6e' }}>ç®åæ²ææå®</div> : heldOrders.map(h => (
              <div key={h.id} style={{ background: '#0d0b09', borderRadius: 12, padding: 12, marginBottom: 8, border: '1px solid #2a2520' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}><span style={{ fontSize: 13, fontWeight: 600, color: '#e8dcc8' }}>{h.table_no || 'ç¡æ¡ä½'} {h.customer_name ? `Â· ${h.customer_name}` : ''}</span><span style={{ fontSize: 11, color: '#8a7e6e' }}>{new Date(h.created_at).toLocaleTimeString('zh-TW', { timeZone: 'Asia/Taipei', hour: '2-digit', minute: '2-digit' })}</span></div>
                <div style={{ fontSize: 11, color: '#8a7e6e', marginBottom: 6 }}>{(h.items_json || []).map(i => `${i.name}Ã${i.qty}`).join('ã')}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => cancelHeldOrder(h)} style={{ padding: isTablet ? '8px 14px' : '6px 12px', fontSize: 11, background: 'rgba(231,76,60,.1)', color: '#e74c3c', border: '1px solid rgba(231,76,60,.3)', borderRadius: 8, cursor: 'pointer' }}>åæ¶</button>
                    <button onClick={() => resumeHeldOrder(h)} style={{ padding: isTablet ? '8px 18px' : '6px 16px', fontSize: 12, fontWeight: 700, background: '#c9a84c', color: '#000', border: 'none', borderRadius: 8, cursor: 'pointer' }}>æ¢å¾©çµå¸³</button>
                  </div>
                </div>
                <div style={{ fontSize: 10, color: '#8a7e6e', marginTop: 4 }}>æ«å­ï¼{h.held_by_name}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {showOrdersModal && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => { setShowOrdersModal(false); setVoidingOrder(null) }}>
        <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, padding: 20, width: '100%', maxWidth: 560, maxHeight: '85vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexShrink: 0 }}><span style={{ fontSize: 16, fontWeight: 700, color: '#c9a84c' }}>ð ä»æ¥è¨å® ({todayOrders.filter(o => o.status === 'completed').length}ç­)</span><button onClick={() => { setShowOrdersModal(false); setVoidingOrder(null) }} style={{ background: 'none', border: 'none', color: '#8a7e6e', cursor: 'pointer' }}><X size={20} /></button></div>
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {!todayOrders.length ? <div style={{ textAlign: 'center', padding: 40, color: '#8a7e6e' }}>ä»æ¥å°ç¡è¨å®</div> : todayOrders.map(o => (
              <div key={o.id} style={{ background: '#0d0b09', borderRadius: 12, padding: 12, marginBottom: 8, border: `1px solid ${o.status === 'voided' ? 'rgba(231,76,60,.3)' : '#2a2520'}`, opacity: o.status === 'voided' ? 0.5 : 1 }}>
                ...
                <div style={{ fontSize: 12, color: '#e8dcc8', marginBottom: 4 }}>{o.items_text || 'â'}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    {o.status === 'voided' && <span style={{ fontSize: 10, color: '#e74c3c', fontWeight: 600 }}>å·²ä½å»¢</span>}
                  </div>
                  {o.status === 'completed' && session?.is_admin && (
                    voidingOrder?.id === o.id ? (
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        <input placeholder="ä½å»¢åå " value={voidReason} onChange={e => setVoidReason(e.target.value)} autoFocus style={{ width: 120, fontSize: 11, padding: '4px 6px', background: '#0d0b09', border: '1px solid #e74c3c', borderRadius: 6, color: '#e8dcc8' }} onKeyDown={e => { if (e.key === 'Enter') doVoidOrder() }} />
                        <button onClick={doVoidOrder} style={{ padding: '4px 10px', fontSize: 10, background: '#e74c3c', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 700 }}>ç¢ºèªä½å»¢</button>
                        <button onClick={() => { setVoidingOrder(null); setVoidReason('') }} style={{ padding: '4px 6px', fontSize: 10, background: 'none', color: '#8a7e6e', border: '1px solid #2a2520', borderRadius: 6, cursor: 'pointer' }}>â</button>
                      </div>
                    ) : (
                      <button onClick={() => setVoidingOrder(o)} style={{ padding: isTablet ? '6px 14px' : '4px 10px', fontSize: 10, background: 'rgba(231,76,60,.1)', color: '#e74c3c', border: '1px solid rgba(231,76,60,.3)', borderRadius: 6, cursor: 'pointer' }}>ä½å»¢</button>
                    )
                  )}
                </div>
              </div>
      ...
      {showShift && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setShowShift(false)}>
          <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 400 }} onClick={e => e.stopPropagation()}>
            {!shift ? <>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#c9a84c', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}><LogIn size={20} /> éç­</div>
              <div style={{ fontSize: 13, color: '#8a7e6e', marginBottom: 6 }}>æä½å¡ï¼<strong style={{ color: '#e8dcc8' }}>{session?.name}</strong></div>
              <div style={{ fontSize: 13, color: '#8a7e6e', marginBottom: 6 }}>åç¨ééé¡</div>
              ...
              <button onClick={openShiftFn} style={{ width: '100%', padding: 14, fontSize: 16, fontWeight: 700, cursor: 'pointer', background: 'linear-gradient(135deg, #4da86c, #2d8a4e)', border: 'none', borderRadius: 12, color: '#fff' }}>ç¢ºèªéç­</button>
            </> : <>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#f59e0b', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}><LogOutIcon size={20} /> éç­</div>
              <div style={{ background: '#0d0b09', borderRadius: 12, padding: 12, marginBottom: 14, fontSize: 13, lineHeight: 2 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#8a7e6e' }}>éç­äººå¡</span><span>{shift.employee_name}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#8a7e6e' }}>åç¨é</span><span style={{ color: '#c9a84c' }}>${(shift.opening_cash || 0).toLocaleString()}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#8a7e6e' }}>ä»æ¥çæ¶</span><span style={{ color: '#4da86c' }}>${(summary?.revenue?.total || 0).toLocaleString()}</span></div>
              </div>
              <div style={{ fontSize: 13, color: '#8a7e6e', marginBottom: 6 }}>ç¾éç¤é»éé¡</div>
              <input type="number" inputMode="numeric" placeholder="ç¤é»ç¾é" value={closingCash} onChange={e => setClosingCash(e.target.value)} style={{ width: '100%', fontSize: 24, fontFamily: 'var(--font-mono)', fontWeight: 700, padding: '12px 16px', textAlign: 'center', background: '#0d0b09', border: '2px solid rgba(245,158,11,.4)', borderRadius: 12, color: '#f59e0b', marginBottom: 16, boxSizing: 'border-box' }} />
              <button onClick={closeShiftFn} style={{ width: '100%', padding: 14, fontSize: 16, fontWeight: 700, cursor: 'pointer', background: 'linear-gradient(135deg, #f59e0b, #d97706)', border: 'none', borderRadius: 12, color: '#000' }}>ç¢ºèªéç­</button>
            </>}
          </div>
        </div>
      )}
