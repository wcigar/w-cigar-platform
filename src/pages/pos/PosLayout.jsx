...
import { DollarSign, Package, LogOut, RefreshCw, Clock, User, ShoppingCart, Printer } from 'lucide-react'
...
export default function PosLayout({ session, shift, summary, cartCount, heldCount, onLogout, onSwitchOperator, onShowHeld, onShowOrders, children }) {
  ...
  useEffect(() => {
    let t; const check = () => getStatus().then(d => setPrinterOk(d.printer === 'online')).catch(() => setPrinterOk(false));
    check(); t = setInterval(check, 30000); return () => clearInterval(t)
  }, [])
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#0d0b09', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        height: 48, minHeight: 48, borderBottom: '1px solid #2a2520',
        flexShrink: 0, background: '#1a1714',
      }}>
        {/* Logo */}
        ...
        <div style={{ display: 'flex', gap: 2, marginLeft: 8 }}>
          {NAV_ITEMS.map(item => {
            const active = location.pathname === item.path || (item.path !== '/pos-app' && location.pathname.startsWith(item.path))
            const Icon = item.icon
            return (
              <button key={item.path} onClick={() => navigate(item.path)}
                style={{
                  display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px',
                  borderRadius: 6, fontSize: 11, fontWeight: active ? 600 : 400, cursor: 'pointer',
        ...
        <div style={{
          ...
          background: '#0d0b09', borderRadius: 8, border: '1px solid #2a2520',
        }}>
      ...
      {showSwitchConfirm && (
        <div style={{
          ...
          <div style={{
            background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 16,
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowSwitchConfirm(false)}
                style={{
                  flex: 1, padding: 12, borderRadius: 10, border: '1px solid #2a2520',
                  background: '#0d0b09', color: '#8a7e6e', fontSize: 14, fontWeight: 600, cursor: 'pointer',
                }}>取消</button>
...
import { useNavigate, useLocation } from 'react-router-dom'
import { DollarSign, Package, LogOut, RefreshCw, Clock, User, ShoppingCart } from 'lucide-react'
