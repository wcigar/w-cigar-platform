...
function NewOrderModal({ employee, onClose, onDone }) {
  ...
  return <Modal onClose={onClose} title="＋ 新增訂單 / 開櫃" wide>
    ...
    {items.map((item,idx) => <div key={idx} style={{ background:'#0d0b09', borderRadius:14, padding:12, marginBottom:10, border:`1px solid ${C.border}` }}>
      <div style={{ display:'flex', gap:6, marginBottom:8 }}><Inp placeholder="搜尋雪茄品名或品牌 *" value={item.name} onChange={e => upd(idx,'name',e.target.value)} style={{ flex:2, marginBottom:0 }} /><Inp type="number" placeholder="單價 $" value={item.price} onChange={e => upd(idx,'price',e.target.value)} style={{ flex:1, marginBottom:0 }} />{items.length>1 && <button onClick={() => setItems(items.filter((_,i)=>i!==idx))} style={{ color:C.danger, background:'none', border:'none', cursor:'pointer', fontSize:16 }}>✕</button>}</div>
