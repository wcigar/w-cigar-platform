// src/pages/admin/VenueSalesDetailed.jsx
// 進階明細模式：每筆銷售逐項商品輸入（原 VenueSalesNew 內容，抽出為獨立檔）
// 用於 /admin/venue-sales/new 的「進階明細模式」tab
...
export default function VenueSalesDetailed() {
    ...
    <>
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map((it, idx) => (
            <ItemRow
              key={it.key} idx={idx} item={it} products={products}
...
function SectionTitle({ children, noMargin }) {
  return <div style={{ fontSize: 11, color: '#8a8278', letterSpacing: 2, marginBottom: noMargin ? 0 : 10 }}>{children}</div>
}
...
function ItemRow({ idx, item, products, onChange, onRemove, canRemove }) {
  ...
  return (
    <div style={{ padding: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 8 }}>
  ...
  return (
    <div style={{ marginTop: 10, padding: '10px 12px', background: 'rgba(255,255,255,0.02)', borderRadius: 6 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        ...
        <span style={{ fontSize: big ? 20 : 15, color, fontWeight: 600 }}>NT$ {Number(value || 0).toLocaleString()}</span>
      </div>
...
function RadioTile({ active, onClick, color, children }) {
  return (
    <button onClick={onClick} style={{
      ...
      color: active ? color : '#8a8278', fontSize: 12, cursor: 'pointer', fontWeight: active ? 600 : 400,
    }}>{children}</button>
  )
}
function ConfirmModal({ sale_date, venue_name, ambassador_name, items, salesTotal, cash, transfer, monthly, unpaid, payment_status, difference, idempotencyKey, onCancel, onConfirm, onConfirmContinue, submitting }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#111', border: '1px solid #c9a84c44', borderRadius: 12, maxWidth: 520, width: '100%', maxHeight: '90vh', overflow: 'auto', padding: 20 }}>
        ...
        {difference !== 0 && (
          <div style={{ marginTop: 10, padding: 10, borderRadius: 6, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.35)', color: '#fde68a', fontSize: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 16 }}>
          ...
          <button onClick={onConfirm} disabled={submitting} style={modalBtn('#10b981', true)}><Check size={14} /> 送出返回</button>
          <button onClick={onConfirmContinue} disabled={submitting} style={modalBtn('#c9a84c', true)}><Check size={14} /> 送出並繼續</button>
        </div>
      </div>
    </div>
  )
...
function modalBtn(color, filled = false) {
  return { padding: '10px 8px', borderRadius: 8, background: filled ? `${color}22` : 'transparent', border: `1px solid ${color}66`, color, cursor: 'pointer', fontSize: 13, fontWeight: 500, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4 }
}
