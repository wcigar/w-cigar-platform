...
// 銷售儀表板 — KPI / 趨勢 / 區域 / 店家 / 大使 / 品項 + 會計視角 + 補貨建議
import { useEffect, useMemo, useState } from 'react'
import { BarChart3, TrendingUp, Users, Building2, RefreshCw, Cigarette, Calculator, Download, Printer, Package } from 'lucide-react'
import PageShell, { Card } from '../../components/PageShell'
import { getSalesReport, getReplenishmentSuggestions, getRegionLabel } from '../../lib/services/salesReport'
...
function DailyLineChart({ data }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', maxWidth: '100%' }}>
      {ticks.map((t, i) => { const y = yScale(t); return <g key={i}><line x1={PADL} y1={y} x2={W-PADR} y2={y} stroke="#1f1c19" strokeWidth="1" /><text x={PADL-6} y={y+4} fill="#8a8278" fontSize="10" textAnchor="end">{t >= 1000 ? (t/1000).toFixed(t >= 10000 ? 0 : 1) + 'k' : t}</text></g> })}
...
function ReplenishmentSuggestion({ replen, loading }) {
  ...
  const { items, lowItems, totalItems, lowCount, criticalCount, totalSuggestedQty } = replen
  ...
  const downloadCSV = () => {
    const lines = [['補貨建議報表', new Date().toISOString().slice(0,10)], [], ['店家', '商品', '類別', '目前庫存', '警戒線', '目標庫存', '建議補貨', '優先度']]
    ...
    a.href = url
    a.download = `w-cigar-補貨建議_${new Date().toISOString().slice(0,10)}.csv`
    a.click()
    ...
    <Card style={{ marginBottom: 12, padding: 14, borderColor: criticalCount > 0 ? '#7f1d1d' : (lowCount > 0 ? '#3a3024' : '#10b981') }} className="no-print">
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
        <h3 style={{ margin: 0, fontSize: 14, color: '#e7e5e4', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          補貨建議
          <span style={{ fontSize: 10, color: '#8a8278', fontWeight: 400, marginLeft: 4 }}>
            ({totalItems} 個 SKU · 警戒 {lowCount} · 緊急 {criticalCount})
          </span>
        </h3>
        {lowCount > 0 && (
          <button onClick={downloadCSV} style={{ marginLeft: 'auto', background: '#1a1815', border: '1px solid #c9a84c', color: '#c9a84c', padding: '4px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <Download size={11} /> 下載補貨單
          </button>
        )}
      </div>
      {allGood ? (
        <div style={{ background: '#0c1a0e', border: '1px solid #166534', color: '#86efac', fontSize: 12, padding: '10px 14px', borderRadius: 6 }}>
          ✅ 目前所有店家庫存充足 — Capadura 系列庫存標準 7 根 / 古巴系列 5 根。員工 KEY 銷量後庫存會自動扣，低於警戒線時會出現在這。
        </div>
      ) : (
        <>
          <div style={{ fontSize: 11, color: '#fca5a5', marginBottom: 8 }}>
            📦 共 {lowCount} 項需補貨，總計 <span style={{ color: '#c9a84c', fontWeight: 600 }}>{totalSuggestedQty} 根</span>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                <th style={{ ...head, textAlign: 'left' }}>店家</th>
                <th style={{ ...head, textAlign: 'left' }}>商品</th>
                <th style={{ ...head, textAlign: 'right' }}>目前</th>
                <th style={{ ...head, textAlign: 'right' }}>警戒</th>
                <th style={{ ...head, textAlign: 'right' }}>目標</th>
                <th style={{ ...head, textAlign: 'right' }}>建議補</th>
                <th style={{ ...head, textAlign: 'center' }}>優先度</th>
              </tr>
            </thead>
            <tbody>
              {lowItems.map((it, i) => (
                <tr key={i} style={{ background: it.is_critical ? 'rgba(220,38,38,0.05)' : 'transparent' }}>
                  ...
                  <td style={cell}>
                    <span style={{ display: 'inline-block', width: 6, height: 6, background: CAT_COLORS[it.category], borderRadius: 2, marginRight: 6 }} />
                    {it.product_label}
                  </td>
                  ...
                  <td style={{ ...cell, textAlign: 'center', fontSize: 10 }}>
                    {it.is_critical
                      ? <span style={{ background: '#7f1d1d', color: '#fca5a5', padding: '2px 6px', borderRadius: 4 }}>緊急</span>
                      : <span style={{ background: '#3a3024', color: '#fbbf24', padding: '2px 6px', borderRadius: 4 }}>低</span>}
                  </td>
                </tr>
              ))}
            </tbody>
...
function AccountingView({ data, from, to }) {
  ...
  useEffect(() => { setShipping(Number(localStorage.getItem('w_shipping_' + pkey) || 0)) }, [pkey])
  function saveShipping(v) { const n = Number(v) || 0; setShipping(n); localStorage.setItem('w_shipping_' + pkey, String(n)) }

  const calc = useMemo(() => {
    if (!data) return null
    const { kpi, byVenue, byAmbassador, byDate, byProductCapadura, byProductCuban } = data
    ...
    return {
      ...
      weekendDates: weekendDates.length, weekdayDates: weekdayDates.length, weekendRev, weekdayRev,
      ...
      byVenue, byAmbassador, byProductCapadura, byProductCuban, kpi,
    }
  }, [data, shipping])
  ...
  const unknownWarn = calc.kpi.qty_unknown_cost > 0 ? `⚠️ ${calc.kpi.unknown_products.join(', ')} 共 ${calc.kpi.qty_unknown_cost} 根尚未填成本，毛利可能高估` : null
    ...
    <Card style={{ marginBottom: 12, padding: 14, borderColor: '#3a3024', background: 'linear-gradient(180deg, #14110d 0%, #0a0a0a 100%)' }} className="accounting-view">
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <input type="number" value={shipping} onChange={e => saveShipping(e.target.value)} style={{ background: '#1a1815', border: '1px solid #2a2824', color: '#fca5a5', padding: '5px 10px', borderRadius: 6, fontSize: 13, width: 120 }} />
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 12, color: '#8a8278', marginBottom: 8, letterSpacing: 1 }}>2. 應收帳款 / 各店毛利明細</div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr><th style={{ ...head, textAlign: 'left' }}>店家</th><th style={{ ...head, textAlign: 'left' }}>地區</th><th style={{ ...head, textAlign: 'right' }}>筆數</th><th style={{ ...head, textAlign: 'right' }}>根數</th><th style={{ ...head, textAlign: 'right' }}>應收金額</th><th style={{ ...head, textAlign: 'right' }}>銷貨成本</th><th style={{ ...head, textAlign: 'right' }}>毛利</th><th style={{ ...head, textAlign: 'right' }}>毛利率</th></tr></thead>
          <tbody>
...
export default function SalesReport() {
  ...
  async function load() {
    ...
    try {
      const [result, replenResult] = await Promise.all([getSalesReport(from, to), getReplenishmentSuggestions().catch(e => null)])
      setData(result)
      setReplen(replenResult)
    } catch (e) {
    ...
    <PageShell title="銷售儀表板" subtitle="ADMIN · SALES REPORT">
      ...
      <Card style={{ marginBottom: 12, padding: 12 }} className="no-print">
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={() => setShowReplen(s => !s)} style={{ ...replenBtn, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <Package size={12} /> 補貨建議{replenLowCount > 0 ? ` (${replenLowCount})` : ''}{showReplen ? '（隱藏）' : ''}
          </button>
