...
// 銷售儀表板 — 即時 KPI / 趨勢 / 區域 / 店家 / 大使 / 品項 + 會計視角(含季對比+外場存貨) + 補貨建議 + 運費紀錄
import { useEffect, useMemo, useState } from 'react'
import { BarChart3, TrendingUp, Users, Building2, RefreshCw, Cigarette, Calculator, Download, Printer, Package, Truck, Calendar, Trash2, Plus, Warehouse } from 'lucide-react'
import PageShell, { Card } from '../../components/PageShell'
import { getSalesReport, getReplenishmentSuggestions, getRegionLabel, getDailyKPI, getQuarterComparison, getShippingLogs, addShippingLog, deleteShippingLog, getOutstandingInventory } from '../../lib/services/salesReport'
...
function QuarterComparison({ qoq }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {noPrev && <div style={{ fontSize: 10, color: '#fbbf24', marginBottom: 6 }}>⚠️ {previous.label} 尚無資料</div>}
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead><tr><th style={{ ...head, textAlign: 'left' }}>指標</th><th style={{ ...head, textAlign: 'right' }}>{previous.label}</th><th style={{ ...head, textAlign: 'right' }}>{current.label}</th><th style={{ ...head, textAlign: 'right' }}>變化</th></tr></thead>
        <tbody>
...
// 外場存貨資產
function OutstandingInventorySection({ inv }) {
  if (!inv) return null
  const { totalQty, totalAsset, totalTargetAsset, byProduct } = inv
  const cell = { padding: '6px 8px', fontSize: 12, color: '#e7e5e4', borderBottom: '1px solid #1f1c19' }
  const head = { ...cell, color: '#8a8278', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, fontWeight: 500 }
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 12, color: '#8a8278', marginBottom: 8, letterSpacing: 1, display: 'flex', alignItems: 'center', gap: 6 }}>
        <Warehouse size={12} /> 外場存貨資產 OUTSTANDING INVENTORY
      </div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
        <Kpi label="外場總根數" value={fmtInt(totalQty)} unit="根" accent="#c9a84c" />
        <Kpi label="外場存貨資產 (現有)" value={fmtMoney(totalAsset)} accent="#10b981" />
        <Kpi label="目標滿庫資產" value={fmtMoney(totalTargetAsset)} accent="#3b82f6" />
      </div>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead><tr><th style={{ ...head, textAlign: 'left' }}>商品</th><th style={{ ...head, textAlign: 'left' }}>類別</th><th style={{ ...head, textAlign: 'right' }}>單根成本</th><th style={{ ...head, textAlign: 'right' }}>外場庫存</th><th style={{ ...head, textAlign: 'right' }}>存貨資產</th></tr></thead>
        <tbody>
          {byProduct.map((p, i) => (
            <tr key={i}>
              <td style={cell}>{p.product_label || p.product_key}</td>
              <td style={cell}><span style={{ display: 'inline-block', width: 6, height: 6, background: CAT_COLORS[p.category], borderRadius: 2, marginRight: 6 }} />{p.category === 'capadura' ? 'Capadura' : '古巴'}</td>
              <td style={{ ...cell, textAlign: 'right', color: '#fca5a5' }}>{fmtMoney(p.cost_per_unit)}</td>
              <td style={{ ...cell, textAlign: 'right' }}>{p.qty} / {p.target_qty}</td>
              <td style={{ ...cell, textAlign: 'right', color: '#10b981' }}>{fmtMoney(p.asset)}</td>
            </tr>
          ))}
          <tr style={{ borderTop: '2px solid #3a3024' }}>
            ...
            <td style={cell}>—</td>
            <td style={cell}></td>
            <td style={{ ...cell, textAlign: 'right', fontWeight: 600 }}>{fmtInt(totalQty)} 根</td>
            <td style={{ ...cell, textAlign: 'right', fontWeight: 600, color: '#10b981' }}>{fmtMoney(totalAsset)}</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <div className="no-print" style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8, alignItems: 'center' }}>
        ...
        <button onClick={submit} disabled={busy || !shipDate || !amount} style={{ background: '#c9a84c', color: '#0a0a0a', border: 'none', padding: '5px 12px', borderRadius: 6, fontSize: 12, fontWeight: 500, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 4 }}><Plus size={11} /> 新增</button>
      </div>
      {loading ? <div style={{ color: '#8a8278', fontSize: 11 }}>載入中…</div>
        : logs.length === 0 ? <div style={{ color: '#8a8278', fontSize: 11 }}>本期間尚無運費紀錄</div>
        : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              {logs.map(l => (
                <tr key={l.id}>
                  ...
                  <td style={{ ...cell, textAlign: 'center' }} className="no-print">
                    <button onClick={() => { if (confirm('刪除這筆運費紀錄？')) onDelete(l.id) }} style={{ background: 'transparent', border: 'none', color: '#dc2626', cursor: 'pointer', padding: 2 }}><Trash2 size={12} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <Card style={{ marginBottom: 12, padding: 14, borderColor: criticalCount > 0 ? '#7f1d1d' : (lowCount > 0 ? '#3a3024' : '#10b981') }} className="no-print">
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
        <h3 style={{ margin: 0, fontSize: 14, color: '#e7e5e4', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}><Package size={14} color={lowCount > 0 ? '#dc2626' : '#10b981'} /> 補貨建議 <span style={{ fontSize: 10, color: '#8a8278', fontWeight: 400 }}>({totalItems} SKU · 警戒 {lowCount} · 緊急 {criticalCount})</span></h3>
              ...
              <tbody>
                {lowItems.map((it, i) => (
                  <tr key={i} style={{ background: it.is_critical ? 'rgba(220,38,38,0.05)' : 'transparent' }}>
                    ...
                    <td style={{ ...cell, textAlign: 'center', fontSize: 10 }}>{it.is_critical ? <span style={{ background: '#7f1d1d', color: '#fca5a5', padding: '2px 6px', borderRadius: 4 }}>緊急</span> : <span style={{ background: '#3a3024', color: '#fbbf24', padding: '2px 6px', borderRadius: 4 }}>低</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function AccountingView({ data, from, to, qoq, shipping, inv }) {
  const calc = useMemo(() => {
    if (!data) return null
    const { kpi, byVenue } = data
...
const printStyle = `@media print { body { background: white !important; } .no-print { display: none !important; } .accounting-view { background: white !important; border: 1px solid #ccc !important; color: black !important; } .accounting-view * { color: black !important; } table { page-break-inside: avoid; } }`
...
export default function SalesReport() {
  ...
  async function loadShipping() {
    ...
    try { const { logs, total } = await getShippingLogs(from, to); setShippingLogs(logs); setShippingTotal(total) } catch (e) { console.error(e) } finally { setShippingLoading(false) }
  }
  async function handleAddShipping(payload) { await addShippingLog({ ...payload, created_by_name: '老闆' }); await loadShipping() }
  async function handleDeleteShipping(id) { await deleteShippingLog(id); await loadShipping() }

  async function load() {
    setLoading(true); setError(null)
    try {
      const [result, replenResult, dailyResult, qoqResult, invResult] = await Promise.all([
        getSalesReport(from, to),
        getReplenishmentSuggestions().catch(() => null),
        getDailyKPI().catch(() => null),
        getQuarterComparison().catch(() => null),
        getOutstandingInventory().catch(() => null),
      ])
      setData(result); setReplen(replenResult); setDaily(dailyResult); setQoq(qoqResult); setInv(invResult)
      await loadShipping()
    } catch (e) { setError(e.message || String(e)); setData(null) } finally { setLoading(false) }
  }
  ...
  return (
    <PageShell title="銷售儀表板" subtitle="ADMIN · SALES REPORT">
      ...
      {showAccounting && <AccountingView data={data} from={from} to={to} qoq={qoq} inv={inv} shipping={{ logs: shippingLogs, total: shippingTotal, loading: shippingLoading, onAdd: handleAddShipping, onDelete: handleDeleteShipping }} />}
