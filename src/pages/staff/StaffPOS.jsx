...
const CATEGORIES = [
  { key: 'all', label: '全部' }, { key: '古巴雪茄', label: '古巴雪茄' }, { key: 'Capadura', label: 'Capadura' },
  { key: '雪茄配件', label: '雪茄配件' }, { key: '莊園品茗', label: '莊園品茗' }, { key: '奶茶咖啡', label: '奶茶咖啡' },
  { key: '氣泡飲品', label: '氣泡飲品' }, { key: '餐食', label: '餐食' }, { key: '甜點', label: '甜點' }, { key: '酒類', label: '酒類' },
]
const CAT_ORDER = {}; CATEGORIES.forEach((c, i) => { CAT_ORDER[c.key] = i })
const SORTS = [{ key: 'menu', label: '菜單順序' }, { key: 'price_desc', label: '價格高→低' }, { key: 'price_asc', label: '價格低→高' }, { key: 'name', label: '名稱' }]
const TIER_STYLES = {
...
function calcMemberDiscount(tier, items) {
  ...
  items.forEach(item => {
    ...
    if (r < 1) { const s = Math.round(item.price * item.qty * (1 - r)); tot += s; d.push({ name: item.name, rate: r, saved: s }) }
  })
...
export default function StaffPOS() {
  ...
  async function holdCurrentOrder() {
    ...
    const { error } = await supabase.from('pos_held_orders').insert({
      table_no: tableNo || null, customer_id: customer?.id || null, customer_name: customer?.name || null, customer_tier: customer?.membership_tier || null,
      attributed_to: customer?.belongs_to || attributedTo,
      items_json: cart.map(c => ({ id: c.id, name: c.name, brand: c.brand, price: c.price, _originalPrice: c._originalPrice, qty: c.qty, _cat: c._cat, inv_master_id: c.inv_master_id, note: c.note, _overridden: c._overridden })),
      cart_count: cart.reduce((s, c) => s + c.qty, 0), subtotal: cart.reduce((s, c) => s + c.price * c.qty, 0),
      discount_pct: discountPct, service_fee_pct: serviceFeePct, notes: orderNote || null,
      held_by: user.employee_id || user.id, held_by_name: user.name,
    })
    if (error) return alert('暫存失敗: ' + error.message)
    clearAll(); loadHeldOrders(); alert('✅ 已暫存')
  }

  async function resumeHeldOrder(held) {
    const items = held.items_json || []
    setCart(items.map(c => ({ ...c, _cartKey: c._customLabel ? c.id + '|' + c._customLabel : c.id, price: c.price || 0, qty: c.qty || 1 })))
    setTableNo(held.table_no || '')
    if (held.customer_id && held.customer_name) {
      setCustomer({ id: held.customer_id, name: held.customer_name, membership_tier: held.customer_tier, belongs_to: held.attributed_to })
      setCustomerTier(tiers.find(t => t.id === held.customer_tier) || null)
    }
    ...
    await supabase.from('pos_held_orders').update({ status: 'resumed', resumed_by: user.employee_id || user.id, resumed_at: new Date().toISOString() }).eq('id', held.id)
    loadHeldOrders(); setShowHeldModal(false); setShowMobileCart(false)
  }

  async function cancelHeldOrder(held) {
    if (!confirm('確定取消此掛單？')) return
    await supabase.from('pos_held_orders').update({ status: 'cancelled' }).eq('id', held.id)
    loadHeldOrders()
  }
  ...
  async function doVoidOrder() {
    ...
    const { data, error } = await supabase.rpc('pos_void_order', { p_order_no: voidingOrder.order_no, p_admin_id: user.employee_id || user.id, p_admin_name: user.name, p_reason: voidReason.trim() })
  ...
  async function doCheckout() {
    if (!cart.length) return; if (payMethod === 'cash' && cashPaid < total) return alert('現金不足')
    setSubmitting(true)
    try {
      ...
      let upg = null
      if (customer) { try { await supabase.from('customers').update({ total_spent: (customer.total_spent || 0) + total }).eq('id', customer.id); const { data: u } = await supabase.rpc('pos_check_member_upgrade', { p_customer_id: customer.id }); if (u?.upgraded) upg = u } catch {} }
      setUpgradeInfo(upg)
  ...
  async function openShiftFn() { const { data } = await supabase.rpc('pos_open_shift', { p_employee_id: user.employee_id || user.id, p_employee_name: user.name, p_opening_cash: +(shiftCash || 0) }); if (data?.success) { setShowShift(false); loadAll() } else alert('開班失敗: ' + (data?.error || '')) }
  async function closeShiftFn() { if (!shift) return; const { data } = await supabase.rpc('pos_close_shift', { p_shift_id: shift.id, p_closing_cash: +(closingCash || 0) }); if (data?.success) { alert('關班完成！差額: $' + (data.variance ?? 0)); setShowShift(false); setClosingCash(''); loadAll() } else alert('關班失敗: ' + (data?.error || '')) }
      ...
      {cart.map(c => { const ck = c._cartKey || c.id; return <div key={ck} style={{ padding: '6px 0', borderBottom: '1px solid #2a2520' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <button onClick={() => updateQty(ck, -1)} style={{ width: isTablet ? 32 : 24, height: isTablet ? 32 : 24, borderRadius: 6, border: '1px solid #2a2520', background: '#0d0b09', color: '#e8dcc8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Minus size={11} /></button>
            <span style={{ width: 22, textAlign: 'center', fontSize: 12, fontWeight: 700, fontFamily: 'var(--font-mono)', color: '#e8dcc8' }}>{c.qty}</span>
            <button onClick={() => updateQty(ck, 1)} style={{ width: isTablet ? 32 : 24, height: isTablet ? 32 : 24, borderRadius: 6, border: '1px solid rgba(201,168,76,.3)', background: 'rgba(201,168,76,.1)', color: '#c9a84c', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Plus size={11} /></button>
          </div>
  ...
  // ── Product card ──
  function PCard({ p }) {
    const inCart = cart.find(c => c.id === p.id), isOut = p._stock === '缺貨', isLow = p._stock === '少量'
    return <button onClick={() => openDetail(p)} disabled={isOut} style={{ background: '#1a1714', border: inCart ? '1.5px solid rgba(201,168,76,.4)' : '1px solid #2a2520', borderRadius: 10, padding: 0, cursor: isOut ? 'not-allowed' : 'pointer', textAlign: 'left', opacity: isOut ? .35 : 1, position: 'relative', overflow: 'hidden', pointerEvents: isOut ? 'none' : 'auto' }}>
      ...
      <div style={{ position: 'relative' }}>
        {p.image_url ? <div style={{ aspectRatio: '4/3', background: '#0f0d0a', overflow: 'hidden' }}><img src={p.image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.style.display = 'none' }} /></div>
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#0d0b09', color: '#e8dcc8' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 10px', height: 50, minHeight: 50, borderBottom: '1px solid #2a2520', flexShrink: 0, background: '#1a1714' }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 700, color: '#c9a84c' }}>POS</span>
        {heldOrders.length > 0 && <span style={{ fontSize: 9, background: 'rgba(245,158,11,.15)', color: '#f59e0b', padding: '2px 6px', borderRadius: 10, cursor: 'pointer' }} onClick={() => { loadHeldOrders(); setShowHeldModal(true) }}>掛單 {heldOrders.length}</span>}
        {shift && <span style={{ fontSize: 9, background: 'rgba(77,168,108,.15)', color: '#4da86c', padding: '2px 6px', borderRadius: 10 }}>營業中</span>}
        <div style={{ flex: 1 }} />
        {!isMobile && <span style={{ fontSize: 11, color: '#8a7e6e' }}>{user?.name}</span>}
        ...
        <button onClick={() => { loadTodayOrders(); setShowOrdersModal(true) }} style={{ background: 'none', border: '1px solid #2a2520', borderRadius: 6, padding: '3px 8px', fontSize: 10, color: '#8a7e6e', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>📋</button>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: isMobile ? 1 : isTablet ? '0 0 55%' : '0 0 65%', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: isMobile ? 'none' : '1px solid #2a2520' }}>
          <div style={{ padding: '5px 8px', borderBottom: '1px solid #2a2520', display: 'flex', gap: 4, alignItems: 'center', flexShrink: 0 }}>
            <div style={{ position: 'relative', flex: 1, minWidth: 80 }}><Search size={13} style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', color: '#8a7e6e' }} /><input placeholder="搜尋品牌 / 品名 / SKU…" value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', fontSize: 12, padding: isTablet ? '8px 8px 8px 32px' : '5px 6px 5px 28px', height: isTablet ? 44 : undefined, background: '#0d0b09', border: '1px solid #2a2520', borderRadius: 8, color: '#e8dcc8' }} /></div>
          ...
          <div style={{ display: 'flex', gap: 2, padding: '3px 8px', overflowX: 'auto', flexShrink: 0, borderBottom: '1px solid #2a2520' }}>
            {CATEGORIES.map(cat => <button key={cat.key} onClick={() => setActiveCategory(cat.key)} style={{ padding: isTablet ? '6px 14px' : '2px 10px', borderRadius: 12, fontSize: isTablet ? 13 : (isMobile ? 10 : 11), fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', background: activeCategory === cat.key ? 'rgba(201,168,76,.15)' : 'transparent', color: activeCategory === cat.key ? '#c9a84c' : '#8a7e6e', border: activeCategory === cat.key ? '1px solid rgba(201,168,76,.3)' : '1px solid transparent' }}>{cat.label}</button>)}
          </div>
          <div style={{ flex: 1, overflow: 'auto', padding: 8 }}>
            <div style={{ display: 'grid', gridTemplateColumns: gridCols, gap: isTablet ? 10 : 6 }}>{filtered.map(p => <PCard key={p.id} p={p} />)}</div>
        ...
        {/* RIGHT: CHECKOUT PANEL (desktop/tablet only) */}
        {!isMobile && <div style={{ flex: isTablet ? '0 0 45%' : '0 0 35%', display: 'flex', flexDirection: 'column', background: '#0d0b09', height: 'calc(100vh - 50px)', overflow: 'hidden' }}>
          {/* Fixed top: table + customer */}
          <div style={{ padding: '8px 12px', borderBottom: '1px solid #2a2520', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              ...
              <div style={{ flex: 1 }} />
              <select value={tableNo} onChange={e => setTableNo(e.target.value)} style={{ fontSize: 11, padding: '4px', background: '#1a1714', border: '1px solid #2a2520', borderRadius: 6, color: '#e8dcc8' }}><option value="">桌位</option>{TABLES.map(t => <option key={t}>{t}</option>)}</select>
            ...
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              {!customer ? <>
                ...
                {canSeeMembers
                  ...
                  : <div onClick={() => { logAudit('blocked_member_access', 'POS 嘗試查看會員（已擋下）', user?.name || user?.employee_id || 'UNKNOWN'); alert('會員資料僅限正職同仁查看') }} style={{ flex: 1, padding: '4px 0', borderRadius: 6, fontSize: 9.5, fontWeight: 600, background: '#15120f', color: '#5a544a', border: '1px dashed #2a2520', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, cursor: 'pointer' }} title="會員資料僅限正職">🔒 會員·限正職</div>}
            ...
            {!customer && <div style={{ display: 'flex', gap: 3, alignItems: 'center' }}><span style={{ fontSize: 9, color: '#8a7e6e', whiteSpace: 'nowrap' }}>歸屬</span>{['老闆', '老闆娘', '店內'].map(a => <button key={a} onClick={() => setAttributedTo(a)} style={{ flex: 1, padding: '2px 0', borderRadius: 6, fontSize: 9, fontWeight: 600, cursor: 'pointer', background: attributedTo === a ? 'rgba(201,168,76,.15)' : '#1a1714', color: attributedTo === a ? '#c9a84c' : '#8a7e6e', border: attributedTo === a ? '1px solid rgba(201,168,76,.3)' : '1px solid #2a2520' }}>{a}</button>)}</div>}
            {customer?.belongs_to && <div style={{ fontSize: 9, color: '#8a7e6e' }}>歸屬：<span style={{ color: '#c9a84c' }}>{customer.belongs_to}</span></div>}
          </div>
          ...
          <div style={{ flexShrink: 0, flexGrow: 0, borderTop: '2px solid rgba(201,168,76,.4)', background: '#0d0b09', padding: '8px 12px' }}>
            <CheckoutBottom />
          </div>
        </div>}
      </div>
      ...
      {isMobile && !showMobileCart && <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', paddingBottom: 'max(10px, env(safe-area-inset-bottom))', borderTop: '2px solid rgba(201,168,76,.4)', background: '#1a1714' }} onClick={() => setShowMobileCart(true)}>
      ...
      {showMobileCart && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }} onClick={() => setShowMobileCart(false)}>
        <div style={{ background: '#1a1714', borderRadius: '16px 16px 0 0', maxHeight: '85vh', display: 'flex', flexDirection: 'column', border: '1px solid rgba(201,168,76,.3)', borderBottom: 'none' }} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ padding: '6px 16px', borderBottom: '1px solid #2a2520', display: 'flex', gap: 4, flexShrink: 0 }}>
            <select value={tableNo} onChange={e => setTableNo(e.target.value)} style={{ flex: 1, fontSize: 12, padding: '5px 6px', background: '#0d0b09', border: '1px solid #2a2520', borderRadius: 8, color: '#e8dcc8' }}><option value="">桌位</option>{TABLES.map(t => <option key={t}>{t}</option>)}</select>
            <input type="number" min={1} value={guestCount} onChange={e => setGuestCount(Math.max(1, +e.target.value || 1))} style={{ width: 50, fontSize: 12, padding: '5px', background: '#0d0b09', border: '1px solid #2a2520', borderRadius: 8, color: '#e8dcc8', textAlign: 'center' }} />
          </div>
      ...
      {lastOrder && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => { setLastOrder(null); setUpgradeInfo(null) }}>
        <div style={{ background: '#1a1714', border: '2px solid rgba(77,168,108,.5)', borderRadius: 20, padding: 30, width: '100%', maxWidth: 400, textAlign: 'center' }} onClick={e => e.stopPropagation()}>
          ...
          {upgradeInfo && <div style={{ background: 'rgba(155,89,182,.1)', border: '1px solid rgba(155,89,182,.3)', borderRadius: 12, padding: 12, marginBottom: 12 }}><div style={{ fontSize: 16, fontWeight: 700, color: '#9b59b6' }}>🎉 恭喜升級！</div><div style={{ fontSize: 13, color: '#e8dcc8', marginTop: 4 }}>{lastOrder.customerName} → <b style={{ color: '#c9a84c' }}>「{upgradeInfo.new_tier}」</b></div></div>}
      ...
      {showHeldModal && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setShowHeldModal(false)}>
        <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, padding: 20, width: '100%', maxWidth: 500, maxHeight: '80vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}><span style={{ fontSize: 16, fontWeight: 700, color: '#f59e0b' }}>⏸ 掛單列表 ({heldOrders.length})</span><button onClick={() => setShowHeldModal(false)} style={{ background: 'none', border: 'none', color: '#8a7e6e', cursor: 'pointer' }}><X size={20} /></button></div>
            ...
            {!heldOrders.length ? <div style={{ textAlign: 'center', padding: 40, color: '#8a7e6e' }}>目前沒有掛單</div> : heldOrders.map(h => (
              <div key={h.id} style={{ background: '#0d0b09', borderRadius: 12, padding: 12, marginBottom: 8, border: '1px solid #2a2520' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => cancelHeldOrder(h)} style={{ padding: '6px 12px', fontSize: 11, background: 'rgba(231,76,60,.1)', color: '#e74c3c', border: '1px solid rgba(231,76,60,.3)', borderRadius: 8, cursor: 'pointer' }}>取消</button>
                    <button onClick={() => resumeHeldOrder(h)} style={{ padding: '6px 16px', fontSize: 12, fontWeight: 700, background: '#c9a84c', color: '#000', border: 'none', borderRadius: 8, cursor: 'pointer' }}>恢復結帳</button>
                  </div>
                </div>
      ...
      {showOrdersModal && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => { setShowOrdersModal(false); setVoidingOrder(null) }}>
        <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, padding: 20, width: '100%', maxWidth: 560, maxHeight: '85vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}><span style={{ fontSize: 16, fontWeight: 700, color: '#c9a84c' }}>📋 今日訂單 ({todayOrders.filter(o => o.status === 'completed').length})</span><button onClick={() => { setShowOrdersModal(false); setVoidingOrder(null) }} style={{ background: 'none', border: 'none', color: '#8a7e6e', cursor: 'pointer' }}><X size={20} /></button></div>
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {!todayOrders.length ? <div style={{ textAlign: 'center', padding: 40, color: '#8a7e6e' }}>今日尚無訂單</div> : todayOrders.map(o => (
              <div key={o.id} style={{ background: '#0d0b09', borderRadius: 12, padding: 12, marginBottom: 8, border: `1px solid ${o.status === 'voided' ? 'rgba(231,76,60,.3)' : '#2a2520'}`, opacity: o.status === 'voided' ? 0.5 : 1 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  {o.status === 'completed' && user?.is_admin && (
                    voidingOrder?.id === o.id ? (
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        ...
                        <button onClick={doVoidOrder} style={{ padding: '4px 8px', fontSize: 10, background: '#e74c3c', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 700 }}>確認</button>
                      ...
                      <button onClick={() => setVoidingOrder(o)} style={{ padding: '4px 10px', fontSize: 10, background: 'rgba(231,76,60,.1)', color: '#e74c3c', border: '1px solid rgba(231,76,60,.3)', borderRadius: 6, cursor: 'pointer' }}>作廢</button>
                    )
                  )}
                </div>
                {o.buyer_name && o.buyer_name !== user?.name && <div style={{ fontSize: 10, color: '#8a7e6e', marginTop: 2 }}>收銀：{o.buyer_name}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      {showShift && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setShowShift(false)}>
        <div style={{ background: '#1a1714', border: '1px solid rgba(201,168,76,.3)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 400 }} onClick={e => e.stopPropagation()}>
          {!shift ? <><div style={{ fontSize: 18, fontWeight: 700, color: '#c9a84c', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}><LogIn size={20} /> 開班</div><div style={{ fontSize: 13, color: '#8a7e6e', marginBottom: 6 }}>備用金金額</div><input type="number" inputMode="numeric" value={shiftCash} onChange={e => setShiftCash(e.target.value)} style={{ width: '100%', fontSize: 24, fontFamily: 'var(--font-mono)', fontWeight: 700, padding: '12px 16px', textAlign: 'center', background: '#0d0b09', border: '2px solid rgba(201,168,76,.3)', borderRadius: 12, color: '#c9a84c', marginBottom: 16, boxSizing: 'border-box' }} /><button onClick={openShiftFn} style={{ width: '100%', padding: 14, fontSize: 16, fontWeight: 700, cursor: 'pointer', background: 'linear-gradient(135deg, #4da86c, #2d8a4e)', border: 'none', borderRadius: 12, color: '#fff' }}>確認開班</button></>
          : <><div style={{ fontSize: 18, fontWeight: 700, color: '#f59e0b', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}><LogOutIcon size={20} /> 關班</div><div style={{ background: '#0d0b09', borderRadius: 12, padding: 12, marginBottom: 14, fontSize: 13, lineHeight: 2 }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#8a7e6e' }}>開班人員</span><span>{shift.employee_name}</span></div><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#8a7e6e' }}>備用金</span><span style={{ color: '#c9a84c' }}>${(shift.opening_cash || 0).toLocaleString()}</span></div><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#8a7e6e' }}>今日營收</span><span style={{ color: '#4da86c' }}>${(summary?.revenue?.total || 0).toLocaleString()}</span></div></div><div style={{ fontSize: 13, color: '#8a7e6e', marginBottom: 6 }}>現金盤點金額</div><input type="number" inputMode="numeric" placeholder="盤點現金" value={closingCash} onChange={e => setClosingCash(e.target.value)} style={{ width: '100%', fontSize: 24, fontFamily: 'var(--font-mono)', fontWeight: 700, padding: '12px 16px', textAlign: 'center', background: '#0d0b09', border: '2px solid rgba(245,158,11,.4)', borderRadius: 12, color: '#f59e0b', marginBottom: 16, boxSizing: 'border-box' }} /><button onClick={closeShiftFn} style={{ width: '100%', padding: 14, fontSize: 16, fontWeight: 700, cursor: 'pointer', background: 'linear-gradient(135deg, #f59e0b, #d97706)', border: 'none', borderRadius: 12, color: '#000' }}>確認關班</button></>}
        </div>
      </div>}
    </div>
  )
