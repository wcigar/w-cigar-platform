...
// Default sort: food/drink first, then cigars
const CAT_SORT_ORDER = { '莊園品茗': 0, '奶茶咖啡': 1, '氣泡飲品': 2, '餐食': 3, '甜點': 4, '酒類': 5, '古巴雪茄': 6, 'Capadura': 7 }
...
// Classify an inventory_master item into a menu category
...
export default function StaffPOS() {
    ...
    try {
      ...
      if (Array.isArray(rpcProducts) && rpcProducts.length > 0 && rpcProducts[0].name) {
        ...
        const { data: dp } = await supabase
          .from('products')
          .select('id, brand, name, spec, pack, price_a, suggest_price, image_url, stock_status, inv_master_id')
        ...
        const bs = new Set(); cigarProducts.forEach(p => { if (p.brand) bs.add(p.brand) })
        rpcBrands = Array.from(bs).sort()
      }
      setBrands(rpcBrands)

      // ── Drinks / food / alcohol — only retail_price > 0 ──
      const { data: invItems } = await supabase
        .from('inventory_master')
        .select('id, name, category, current_stock, safe_stock, retail_price, unit, image_url')
        .eq('enabled', true)
        .in('category', ['吧台飲品', '餐飲', '酒類'])
        .gt('retail_price', 0)
  ...
  const filtered = useMemo(() => {
    ...
    // Sort
    if (sortBy === 'menu') {
      list = [...list].sort((a, b) => (CAT_SORT_ORDER[a._cat] ?? 99) - (CAT_SORT_ORDER[b._cat] ?? 99) || (a.name || '').localeCompare(b.name || ''))
    } else if (sortBy === 'name') {
      ...
      <div className="pos-main" style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div className="pos-products" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', gap: 2, padding: '3px 8px', overflowX: 'auto', flexShrink: 0, borderBottom: '1px solid var(--border)' }}>
            ...
            {/* Brand sub-pills for cigar tabs */}
            ...
            {viewMode === 'grid' ? (
                ...
                {filtered.map(p => {
                  ...
                  return (
                    <button key={p.id} onClick={() => openDetail(p)} disabled={isOut}
                      style={{ background: inCart ? 'rgba(201,168,76,.08)' : 'var(--black-card)', border: inCart ? '1.5px solid var(--border-gold)' : '1px solid var(--border)', borderRadius: 10, padding: 0, cursor: isOut ? 'not-allowed' : 'pointer', textAlign: 'left', opacity: isOut ? .4 : 1, position: 'relative', transition: 'all .15s', overflow: 'hidden', pointerEvents: isOut ? 'none' : 'auto' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {filtered.map(p => {
                  ...
                  return (
                    <button key={p.id} onClick={() => openDetail(p)} disabled={isOut}
                      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: inCart ? 'rgba(201,168,76,.08)' : 'var(--black-card)', border: inCart ? '1.5px solid var(--border-gold)' : '1px solid var(--border)', borderRadius: 10, cursor: isOut ? 'not-allowed' : 'pointer', opacity: isOut ? .4 : 1, textAlign: 'left', width: '100%', position: 'relative', pointerEvents: isOut ? 'none' : 'auto' }}>
        ...
        <div className="pos-cart-panel" style={{ width: 340, display: 'flex', flexDirection: 'column', background: 'var(--black-card)', overflow: 'hidden' }}>
            ...
            {cart.length === 0 ? (
              ...
              <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>${c.price.toLocaleString()} × {c.qty}</div>
                </div>
          ...
          <div style={{ flexShrink: 0, borderTop: '1px solid var(--border-gold)', background: 'rgba(201,168,76,.04)', padding: '8px 14px', display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            {invoiceEnabled && (
              <div style={{ display: 'flex', gap: 4 }}>
                <input value={taxId} onChange={e => setTaxId(e.target.value)} placeholder="統一編號" style={{ flex: 1, fontSize: 10, padding: '3px 4px', background: 'var(--black)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text)' }} />
                <input value={carrier} onChange={e => setCarrier(e.target.value)} placeholder="載具" style={{ flex: 1, fontSize: 10, padding: '3px 4px', background: 'var(--black)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text)' }} />
              </div>
            )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setDetailProduct(null)}>
          <div style={{ background: 'var(--black-card)', border: '1px solid var(--border-gold)', borderRadius: 20, width: '100%', maxWidth: 400, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '16px 20px' }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>{detailProduct.brand} · {detailProduct._cat}</div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: detailProduct._stock === '現貨' ? 'var(--green)' : detailProduct._stock === '少量' ? '#f59e0b' : 'var(--red)' }}>{detailProduct._stock}</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 4, justifyContent: 'center', marginBottom: 16 }}>
                {QTY_PRESETS.map(n => (
                  <button key={n} onClick={() => setDetailQty(n)} style={{ padding: '4px 14px', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: detailQty === n ? 'var(--gold-glow)' : 'var(--black)', color: detailQty === n ? 'var(--gold)' : 'var(--text-dim)', border: detailQty === n ? '1px solid var(--border-gold)' : '1px solid var(--border)' }}>{n}</button>
                ))}
              </div>
          ...
          <div style={{ background: 'var(--black-card)', border: '1px solid var(--border-gold)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            {payMethod === 'cash' && (
              <div style={{ marginBottom: 14 }}>
                ...
                <input type="number" inputMode="numeric" placeholder="輸入金額" value={payAmount} onChange={e => setPayAmount(e.target.value)} autoFocus
      ...
      {lastOrder && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setLastOrder(null)}>
          <div style={{ background: 'var(--black-card)', border: '2px solid rgba(77,168,108,.5)', borderRadius: 20, padding: 30, width: '100%', maxWidth: 380, textAlign: 'center' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 15, fontFamily: 'var(--font-mono)', color: 'var(--gold)', fontWeight: 700, marginBottom: 4 }}>{lastOrder.order_no}</div>
            <div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 16 }}>{PAY_METHODS.find(m => m.key === lastOrder.payMethod)?.label || lastOrder.payMethod}</div>
