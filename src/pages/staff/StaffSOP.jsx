...
export default function StaffSOP() {
  ...
  async function loadTasks() {
    setLoading(true)
    const { data } = await supabase.from('task_status').select('*').eq('owner', user.employee_id).eq('date', today).order('task_id')
    setTasks(data || [])
    setLoading(false)
  }
  ...
  async function toggleTask(task) {
    ...
    if (!task.completed) {
      ...
      update.completed_by = user.name
      update.completed_by_id = user.employee_id
    } else {
    ...
    <div className="page-container fade-in">
      ...
      {tasks.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {tasks.map(t => (
            <div key={t.id} className="card" style={{ padding: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12 }} onClick={() => toggleTask(t)}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {t.completed_at && <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}><Clock size={10}/> {format(new Date(t.completed_at), 'HH:mm')} 完成</div>}
              </div>
            </div>
          ))}
        </div>
