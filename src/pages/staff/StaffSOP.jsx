...
export default function StaffSOP() {
  ...
  async function handleSubmit() {
    ...
    for (const taskId of checkedIds) {
      ...
      if (def.need_photo && !photos[taskId] && !task?.photo_url) {
        return alert(`「${def.title || taskId}」需要拍照`)
      }
    ...
    for (const taskId of checkedIds) {
      try {
        ...
        // Check grab conflict
        if (task.owner === 'ALL' && task.completed && task.completed_by && task.completed_by !== user.name) {
          errors.push(`「${task.title}」已被 ${task.completed_by} 搶走`)
          continue
        }

        let photoUrl = task.photo_url || ''
        if (photos[taskId]) {
          try { photoUrl = await uploadPhoto(taskId, photos[taskId]) } catch (e) { errors.push(`照片上傳失敗: ${e.message}`) }
        }
        ...
        await supabase.from('task_status').update({
          ...
          completed_by_id: user.employee_id,
          photo_url: photoUrl,
          humidor_temp: h.ht || '',
          humidor_rh: h.hr || '',
          cabinet_temp: h.ct || '',
  ...
  async function recallTask(task) {
    ...
    await supabase.from('task_status').update({
      completed: false, completed_at: null, completed_by: '', completed_by_id: '', photo_url: ''
    }).eq('id', task.id)
    load()
  }
  ...
  return (
    <div className="page-container fade-in">
      ...
      {Object.entries(cats).map(([cat, items]) => (
        <div key={cat}>
          ...
          {items.map(t => <TaskCard key={t.id} task={t} def={getDef(t.task_id)} user={user}
            ...
            photo={photos[t.task_id]} onPhoto={f => setPhotos(p=>({...p,[t.task_id]:f}))}
      ...
      {grabTasks.length > 0 && (
        <>
          <div style={{fontSize:14,fontWeight:700,color:'var(--green)',marginTop:20,marginBottom:8,display:'flex',alignItems:'center',gap:6}}>
            🏆 搶單任務 ({grabTasks.filter(t=>t.completed).length}/{grabTasks.length})
          </div>
      ...
      {/* Submit button */}
      <button className="btn-gold" style={{width:'100%',fontSize:18,marginTop:20,padding:16,display:'flex',alignItems:'center',justifyContent:'center',gap:8,opacity:submitting?.6:1}}
...
function TaskCard({ task: t, def, user, isGrab, checked, onCheck, photo, onPhoto, note, onNote, humData, onHumData, expanded, onExpand, onRecall }) {
  ...
  return (
    <div className="card" style={{padding:0,marginBottom:6,overflow:'hidden',borderColor:checked?'var(--border-gold)':t.completed?'rgba(77,168,108,.2)':undefined,background:checked?'rgba(201,168,76,.04)':undefined,opacity:t.completed?.65:1}}>
      {/* Main row */}
      <div style={{padding:'12px 14px',display:'flex',alignItems:'center',gap:10,cursor:'pointer'}} onClick={onExpand}>
        {canEdit ? (
          <input type="checkbox" checked={!!checked} onChange={e => {e.stopPropagation(); onCheck(e.target.checked)}}
            style={{width:24,height:24,accentColor:'#c9a84c',flexShrink:0}} onClick={e=>e.stopPropagation()}/>
        ) : t.completed ? (
          <CheckCircle2 size={22} color="var(--green)" style={{flexShrink:0}}/>
        ) : <Circle size={22} color="var(--text-muted)" style={{flexShrink:0}}/>}

        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,fontWeight:600,color:t.completed?'var(--text-dim)':'var(--text)',textDecoration:t.completed?'line-through':'none',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.title}</div>
          <div style={{fontSize:11,color:'var(--text-muted)',display:'flex',gap:6,flexWrap:'wrap',marginTop:2}}>
            ...
            {needPhoto && <span style={{color:'var(--red)'}}>📷需拍照</span>}
        ...
        <div style={{flexShrink:0,display:'flex',gap:4,alignItems:'center'}}>
          {t.completed && t.completed_by === user.name && (
            <button className="btn-outline" style={{padding:'4px 8px',fontSize:11,color:'var(--red)',borderColor:'rgba(196,77,77,.3)'}} onClick={e=>{e.stopPropagation();onRecall()}}>
              <RotateCcw size={12}/> 撤回
            </button>
          )}
          {photo && <span style={{color:'var(--green)',fontSize:11}}>📷已選</span>}
        ...
        <div style={{padding:'0 14px 14px',borderTop:'1px solid var(--border)'}}>
          ...
          {needPhoto && (
            <div style={{marginTop:8}}>
              ...
              <button className="btn-outline" style={{display:'flex',alignItems:'center',gap:6,fontSize:13,padding:'8px 14px',width:'100%',justifyContent:'center',background:photo?'rgba(77,168,108,.08)':undefined,borderColor:photo?'rgba(77,168,108,.3)':undefined,color:photo?'var(--green)':undefined}}
                onClick={() => fileRef.current?.click()}>
                <Camera size={16}/> {photo ? `已選擇: ${photo.name.slice(0,20)}` : '拍照 / 選擇照片'}
              </button>
              {t.photo_url && <a href={t.photo_url} target="_blank" style={{fontSize:11,color:'var(--blue)',marginTop:4,display:'block'}}>查看已上傳照片</a>}
            </div>
          )}
