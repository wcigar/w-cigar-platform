  ...
  return (
    <div style={styles.wrapper}>
      ...
      <div style={styles.container} className="fade-in">
        ...
        <div style={styles.footer}>CAPADURA · 雪茄紳士俱樂部</div>
      </div>
    </div>
  )
}
...
const styles = {
  ...
  bgNoise: { position: 'absolute', inset: 0, opacity: 0.03, backgroundImage: "url(\"data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E\")" },
  bgGradient: { position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 30%, rgba(201,168,76,0.06) 0%, transparent 60%)' },
  bgVignette: { position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at center, transparent 40%, rgba(0,0,0,0.6) 100%)' },
  container: { position: 'relative', zIndex: 1, width: '100%', maxWidth: 380, padding: '0 24px' },
  logoSection: { textAlign: 'center', marginBottom: 40 },
  logoMark: { fontFamily: 'var(--font-display)', fontSize: 72, fontWeight: 700, color: '#c9a84c', lineHeight: 1, marginBottom: 8, textShadow: '0 0 40px rgba(201,168,76,0.3)' },
  ...
  input: { background: 'rgba(26,26,26,0.8)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 10, padding: '14px 16px', fontSize: 16, color: '#e8e0d0', outline: 'none', fontFamily: 'var(--font-body)', transition: 'border-color 0.2s', width: '100%', WebkitAppearance: 'none', appearance: 'none' },
  error: { color: '#c44d4d', fontSize: 13, textAlign: 'center', padding: '8px 0' },
  loginBtn: { background: 'linear-gradient(135deg, #c9a84c, #8b7a3e)', color: '#0a0a0a', fontWeight: 700, padding: '16px', borderRadius: 10, fontSize: 15, letterSpacing: 2, cursor: 'pointer', border: 'none', fontFamily: 'var(--font-body)', marginTop: 8, transition: 'all 0.2s' },
