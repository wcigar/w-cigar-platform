...
import { AlertTriangle, Phone, Camera, Send, X } from 'lucide-react'
...
export default function AbnormalReport({ show, onClose }) {
  ...
  async function send() {
    ...
    await supabase.from('abnormal_reports').insert({
      date: today, reporter: user.name, description: note, photo_url: photoUrl, status: '待處理'
    })
    setNote(''); setPhoto(null); setSending(false)
    alert('異常回報已送出！老闆會收到通知')
    onClose()
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.8)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={onClose}>
      <div style={{ background: 'var(--black-card)', border: '1px solid var(--border-gold)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 440, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--red)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <AlertTriangle size={22} /> 突發異常回報
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 16 }}>
          {VENDORS.map(v => (
            <a key={v.tel} href={`tel:${v.tel}`} style={{
              display: 'block', padding: '8px 10px', background: 'rgba(255,255,255,.03)',
              border: '1px solid var(--border)', borderRadius: 10, color: 'var(--text-dim)',
              textDecoration: 'none', fontSize: 12, lineHeight: 1.4
            }}>
              {v.icon} {v.name}
              <div style={{ color: 'var(--green)', fontWeight: 700, marginTop: 2 }}>{v.display}</div>
            </a>
          ))}
        </div>
        ...
        <textarea
          placeholder="例如：酒杯打破、馬桶不通、鐵捲門故障..."
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button className="btn-gold" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'linear-gradient(135deg, #c44d4d, #a03333)', opacity: sending ? .6 : 1 }}
