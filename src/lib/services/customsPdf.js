...
export function makePackingList({ supplier, shipment }) {
  ...
  autoTable(doc, {
    ...
    body: items.map(it => [ascii(it.name + (it.unit_weight_g && it.total_pcs ? ' (Net ' + (it.unit_weight_g * it.total_pcs).toFixed(0) + 'g)' : '')), it.hs_code || shipment.hs_code || '2402.10.00.00-8', it.qty_bundles, it.pcs_per_bundle, it.total_pcs, it.package_type || 'Bundle']),
...
export function makeCertificateOfOrigin({ supplier, shipment }) {
  ...
  autoTable(doc, {
    ...
    body: items.map(it => [ascii(it.name + (it.unit_weight_g && it.total_pcs ? ' (Net ' + (it.unit_weight_g * it.total_pcs).toFixed(0) + 'g)' : '')), it.qty_bundles, it.pcs_per_bundle, it.total_pcs]),
...
export function makeCommercialInvoice({ supplier, shipment }) {
  ...
  autoTable(doc, {
    ...
    body: items.map(it => [
      ascii(it.name + (it.unit_weight_g && it.total_pcs ? ' (Net ' + (it.unit_weight_g * it.total_pcs).toFixed(0) + 'g)' : '')),
