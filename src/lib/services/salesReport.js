...
// 銷售儀表板 — 撈 venue_sales_daily 區間，本地端 aggregate（含 COGS 毛利 + 補貨 + 運費 + 季對比 + 即時KPI）
...
export async function getQuarterComparison() {
  ...
  const [curStats, prevStats] = await Promise.all([fetch(cur), fetch(prev)])
