...
// 銷售儀表板 — 撈 venue_sales_daily 區間，本地端 aggregate（含 COGS 毛利 + 補貨建議）
...
// 補貨建議：撈 inventory_balances，標記低於警戒
export async function getReplenishmentSuggestions() {
  ...
  const items = (invRes.data || []).map(it => {
    ...
    return {
      ...
      product_label: PRODUCT_LABEL[it.product_key] || it.product_key,
  ...
  const criticalCount = items.filter(x => x.is_critical).length
  const totalSuggestedQty = lowItems.reduce((s, x) => s + x.suggested_qty, 0)
  return { items, lowItems, totalItems, lowCount, criticalCount, totalSuggestedQty }
}
